...
| 模块          | 路径                                                         | 功能                                                                                                         |
| ------------- | ------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------ |
| Java 切面日志 | `src/java/isolation_validation/LoggingAspect.java`           | 通过 AspectJ 在每次方法调用前后写出入参 / 返回 / 静态字段快照                                                |
| 字符串转换器  | `src/java/isolation_validation/CustomToStringConverter.java` | 把任意对象序列化为 JSON，供日志解析                                                                          |
| pom 改造      | `src/java/isolation_validation/modify_pom.py`                | 给 Java 项目注入 AspectJ + JUnit 依赖与插件                                                                  |
| Aspect 复制   | `src/java/isolation_validation/add_java_files.py`            | 把 LoggingAspect / CustomToStringConverter 拷到每个 Java 包                                                  |
| 日志解析      | `src/java/isolation_validation/log_parser.py`                | 把切面日志拆成结构化 workflow（Args/Return/Instance/Static）                                                 |
| 测试 emitter  | `src/java/isolation_validation/script.py`                    | 由 workflow 生成 `_test.cj` + `.workflow.json`，含 focal call / mock 桩                                      |
| Mock 渲染辅助 | `src/java/isolation_validation/mock_helper.py`               | 渲染 `@On` mock、断言、值构造、in-place mutation 等                                                          |
| 反射类型映射  | `src/java/isolation_validation/reflection.py`                | Java 反射类型 → Cangjie 类型映射                                                                             |
| 修饰符切换    | `src/java/isolation_validation/change_mode.py`               | apply：私有字段 / init 临时改 public 并埋桩；restore：按桩还原                                               |
| 副作用插桩    | `src/java/isolation_validation/side_effect.py`               | 在 focal method 的 callee 调用点注入副作用回放语句                                                           |
| 运行时辅助    | `src/java/isolation_validation/runtime_support.py`           | 注入 / 清理 `helper.cj` 与 `simple_ioc.cj`                                                                   |
| 主流程接入    | `src/java/isolation_validation/test_runner.py`               | 翻译会话级 inject/clean + per-fragment apply→test→restore，被 `compositional_translation_validation.py` 调用 |
...
│   └── isolation_validation/    # Mock 测试 / 隔离验证
│       ├── LoggingAspect.java         # AspectJ 切面：方法级日志
│       ├── CustomToStringConverter.java
│       ├── helper_template.cj.tmpl    # helper.cj 模板（含 __PACKAGE__）
│       ├── modify_pom.py              # Java 项目 pom 改造
│       ├── add_java_files.py          # 拷贝 LoggingAspect 到各 Java 包
│       ├── log_parser.py              # 切面日志 → workflow 结构化
│       ├── script.py                  # workflow → _test.cj + workflow.json emitter
│       ├── mock_helper.py             # mock / 断言 / 值构造渲染
│       ├── reflection.py              # Java 反射类型 → Cangjie 映射
│       ├── change_mode.py             # 修饰符 apply / restore
│       ├── side_effect.py             # focal method 调用点副作用插桩
│       ├── runtime_support.py         # 注入 / 清理 helper.cj、simple_ioc.cj
│       └── test_runner.py             # 主流程接入：会话级 inject/clean + per-fragment 测试循环
