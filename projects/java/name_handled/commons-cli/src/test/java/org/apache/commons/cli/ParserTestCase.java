...
public abstract class ParserTestCase {
    ...
    private CommandLine parse(
            final CommandLineParser parser_param,
            final Options opts,
            final String[] args,
            final Properties properties)
            throws ParseException {
        if (parser_param instanceof Parser) {
            return ((Parser) parser_param).parse2(opts, args, properties);
        }
        if (parser_param instanceof DefaultParser) {
            return ((DefaultParser) parser_param).parse2(opts, args, properties);
        }
