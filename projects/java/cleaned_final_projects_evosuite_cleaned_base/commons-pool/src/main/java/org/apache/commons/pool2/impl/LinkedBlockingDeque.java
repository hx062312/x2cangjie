...
class LinkedBlockingDeque<E> extends AbstractQueue<E> implements Deque<E>, Serializable {
     ...
     * keep all Nodes GC-reachable from a predecessor dequeued LinkedBlockingDeque_Node.
     * That would cause two problems:
     * - allow a rogue Iterator to cause unbounded memory retention
     * - cause cross-generational linking of old Nodes to new Nodes if
     *   a LinkedBlockingDeque_Node was tenured while live, which generational GCs have a
     *   hard time dealing with, causing repeated major collections.
     * However, only non-deleted Nodes need to be reachable from
     * dequeued Nodes, and reachability does not necessarily have to
     * be of the kind understood by the GC.  We use the trick of
     * linking a LinkedBlockingDeque_Node that has just been dequeued to itself.  Such a
    ...
    private abstract class LinkedBlockingDeque_AbstractItr implements Iterator<E> {
        /** The next node to return in next() */
        LinkedBlockingDeque_Node<E> next;
        ...
        /**
         * LinkedBlockingDeque_Node returned by most recent call to next. Needed by remove. Reset to null if this
         * element is deleted by a call to remove.
         */
        private LinkedBlockingDeque_Node<E> lastRet;

        /** Constructs a new iterator. Sets the initial position. */
        LinkedBlockingDeque_AbstractItr() {
            lock.lock();
            try {
                next = firstNode();
         ...
         * @return first node
         */
        abstract LinkedBlockingDeque_Node<E> firstNode();
         ...
         * @return next node
         */
        abstract LinkedBlockingDeque_Node<E> nextNode(LinkedBlockingDeque_Node<E> n);

        @Override
        public void remove() {
            final LinkedBlockingDeque_Node<E> n = lastRet;
            if (n == null) {
         ...
         */
        private LinkedBlockingDeque_Node<E> succ(LinkedBlockingDeque_Node<E> n) {
            for (; ; ) {
                final LinkedBlockingDeque_Node<E> s = nextNode(n);
                if (s == null) {
                    return null;
                }
                if (s.item != null) {
    ...
    /** Descending iterator */
    private class LinkedBlockingDeque_DescendingItr extends LinkedBlockingDeque_AbstractItr {
        @Override
        LinkedBlockingDeque_Node<E> firstNode() {
            return last;
        }

        @Override
        LinkedBlockingDeque_Node<E> nextNode(final LinkedBlockingDeque_Node<E> n) {
            return n.prev;
        }
    }

    /** Forward iterator */
    private class LinkedBlockingDeque_Itr extends LinkedBlockingDeque_AbstractItr {
        @Override
        LinkedBlockingDeque_Node<E> firstNode() {
            return first;
        }
     ...
     */
    private static final class LinkedBlockingDeque_Node<E> {
        /** The item, or null if this node has been removed. */
        E item;

        /**
         * One of: - the real predecessor LinkedBlockingDeque_Node - this LinkedBlockingDeque_Node, meaning the predecessor is tail - null,
         * meaning there is no predecessor
         */
        LinkedBlockingDeque_Node<E> prev;

        /**
         * One of: - the real successor LinkedBlockingDeque_Node - this LinkedBlockingDeque_Node, meaning the successor is head - null,
         ...
         * @param n Next item
         */
        LinkedBlockingDeque_Node(final E x, final LinkedBlockingDeque_Node<E> p, final LinkedBlockingDeque_Node<E> n) {
            item = x;
            prev = p;
            next = n;
        }
     ...
     * first.item != null)
     */
    private transient LinkedBlockingDeque_Node<E> first; // @GuardedBy("lock")

    /**
     * Pointer to last node. Invariant: (first == null && last == null) || (last.next == null &&
     * last.item != null)
     */
    private transient LinkedBlockingDeque_Node<E> last; // @GuardedBy("lock")
     ...
     */
    public static LinkedBlockingDeque LinkedBlockingDeque3(final int capacity_param) {
        return new LinkedBlockingDeque<>(0, capacity_param, false, null);
    }
    ...
    public void clear() {
        lock.lock();
        try {
            for (LinkedBlockingDeque_Node<E> f = first; f != null; ) {
                f.item = null;
                final LinkedBlockingDeque_Node<E> n = f.next;
                f.prev = null;
                f.next = null;
                f = n;
            }
    ...
    public boolean contains(final Object o) {
        ...
        lock.lock();
        try {
            for (LinkedBlockingDeque_Node<E> p = first; p != null; p = p.next) {
                if (o.equals(p.item)) {
                    return true;
                }
            }
    ...
    public Iterator<E> descendingIterator() {
        return new LinkedBlockingDeque_DescendingItr();
    }
    ...
    public Iterator<E> iterator() {
        return new LinkedBlockingDeque_Itr();
    }
    ...
    private boolean linkFirst(final E e) {
        ...
        final LinkedBlockingDeque_Node<E> f = first;
        final LinkedBlockingDeque_Node<E> x = new LinkedBlockingDeque_Node<>(e, null, f);
        first = x;
        if (last == null) {
            last = x;
        } else {
    ...
    private boolean linkLast(final E e) {
        ...
        final LinkedBlockingDeque_Node<E> l = last;
        final LinkedBlockingDeque_Node<E> x = new LinkedBlockingDeque_Node<>(e, l, null);
        last = x;
        if (first == null) {
            first = x;
        } else {
    ...
    public boolean removeLastOccurrence(final Object o) {
        ...
        lock.lock();
        try {
            for (LinkedBlockingDeque_Node<E> p = last; p != null; p = p.prev) {
                if (o.equals(p.item)) {
                    unlink(p);
                    return true;
                }
     ...
     */
    private void unlink(final LinkedBlockingDeque_Node<E> x) {
        final LinkedBlockingDeque_Node<E> p = x.prev;
        final LinkedBlockingDeque_Node<E> n = x.next;
        if (p == null) {
            unlinkFirst();
        } else if (n == null) {
            unlinkLast();
    ...
    private E unlinkLast() {
        ...
        final LinkedBlockingDeque_Node<E> p = l.prev;
        final E item = l.item;
        l.item = null;
        l.prev = l; // help GC
        last = p;
