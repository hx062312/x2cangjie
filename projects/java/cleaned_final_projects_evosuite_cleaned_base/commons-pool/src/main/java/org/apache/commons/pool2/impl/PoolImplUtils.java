...
class PoolImplUtils {
    ...
    static Class<?> getFactoryType(final Class<? extends PooledObjectFactory> factoryClass) {
        final Class<PooledObjectFactory> type__ = PooledObjectFactory.class;
        final Object genericType = getGenericType(type__, factoryClass);
        if (genericType instanceof Integer) {
            final ParameterizedType pi = getParameterizedType(type__, factoryClass);
            if (pi != null) {
                final Type[] bounds =
                        ((TypeVariable)
     ...
     */
    private static <T> Object getGenericType(final Class<T> type__, final Class<? extends T> clazz) {
        if (type__ == null || clazz == null) {
            return null;
        }

        final ParameterizedType pi = getParameterizedType(type__, clazz);
        if (pi != null) {
        ...
        final Object result = getGenericType(type__, superClass);
