...
class EvictionTimer {
     ...
     * live tasks left.
     */
    private static class EvictionTimer_Reaper implements Runnable {
        @Override
        public void run() {
            synchronized (EvictionTimer.class) {
                for (final Entry<WeakReference<Runnable>, EvictionTimer_WeakRunner> entry : taskMap.entrySet()) {
     ...
     */
    private static class EvictionTimer_WeakRunner implements Runnable {
         ...
         */
        private EvictionTimer_WeakRunner(final WeakReference<Runnable> ref_param) {
            this.ref = ref_param;
        }
    ...
    private static final HashMap<WeakReference<Runnable>, EvictionTimer_WeakRunner> taskMap =
    ...
    static synchronized void cancel(
            final BaseGenericObjectPool<?>.BaseGenericObjectPool_Evictor evictor,
     ...
     */
    private static void remove(final BaseGenericObjectPool<?>.BaseGenericObjectPool_Evictor evictor) {
        for (final Entry<WeakReference<Runnable>, EvictionTimer_WeakRunner> entry : taskMap.entrySet()) {
     ...
     * method *must* call {@link #cancel(BaseGenericObjectPool.BaseGenericObjectPool_Evictor, Duration, boolean)} to
