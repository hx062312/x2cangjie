...
public final class CSVPrinter implements Flushable, Closeable {
     ...
     */
    public CSVPrinter(final Appendable appendable_param, final CSVFormat format_param) throws IOException {
        Objects.requireNonNull(appendable_param, "appendable");
        Objects.requireNonNull(format_param, "format");

        this.appendable = appendable_param;
        this.format = format_param.copy();
        final String[] headerComments = format_param.getHeaderComments();
        if (headerComments != null) {
            for (final String line : headerComments) {
                this.printComment(line);
            }
        }
        if (format_param.getHeader() != null && !format_param.getSkipHeaderRecord()) {
            this.printRecord1((Object[]) format_param.getHeader());
        }
    }
