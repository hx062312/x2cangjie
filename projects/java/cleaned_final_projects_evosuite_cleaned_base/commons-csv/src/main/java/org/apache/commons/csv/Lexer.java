...
final class Lexer implements Closeable {
    ...
    Lexer(final CSVFormat format, final ExtendedBufferedReader reader_param) {
        this.reader = reader_param;
    ...
    Token nextToken(final Token token) throws IOException {
        ...
        if (isStartOfLine(lastChar) && isCommentStart(c)) {
            ...
            token.type__ = COMMENT;
            return token;
        }

        while (token.type__ == INVALID) {
