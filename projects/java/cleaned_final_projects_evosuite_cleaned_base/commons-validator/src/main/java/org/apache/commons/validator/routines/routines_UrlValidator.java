 ...
 * <pre>
 *   Example of usage:
 *   Construct a routines_UrlValidator with valid schemes of "http", and "https".
 *
 *    String[] schemes = {"http","https"}.
 *    routines_UrlValidator urlValidator = new routines_UrlValidator(schemes);
 ...
 *
 *    routines_UrlValidator urlValidator = new routines_UrlValidator();
 *    if (urlValidator.isValid("ftp://foo.bar.com/")) {
 *       System.out.println("url is valid");
 *    } else {
 *       System.out.println("url is invalid");
 ...
 * @since Validator 1.4
 */
public class routines_UrlValidator implements Serializable {
     ...
     * validator with a {@link RegexValidator} instead ({@link #routines_UrlValidator(RegexValidator, long)})
     */
    ...
    private static final routines_UrlValidator DEFAULT_URL_VALIDATOR = routines_UrlValidator.UrlValidator6();
     ...
     */
    public static routines_UrlValidator getInstance() {
        return DEFAULT_URL_VALIDATOR;
    }

    private final DomainValidator domainValidator;

    /** Create a routines_UrlValidator with default properties. */
    public routines_UrlValidator(
            String[] schemes,
            RegexValidator authorityValidator_param,
            long options_param,
            DomainValidator domainValidator_param) {
        this.options = options_param;
        if (domainValidator_param == null) {
            throw new IllegalArgumentException("DomainValidator must not be null");
        }
        if (domainValidator_param.isAllowLocal() != ((options_param & ALLOW_LOCAL_URLS) > 0)) {
            throw new IllegalArgumentException(
                    "DomainValidator disagrees with ALLOW_LOCAL_URLS setting");
        }
        this.domainValidator = domainValidator_param;
        ...
        this.authorityValidator = authorityValidator_param;
    }

    public static routines_UrlValidator UrlValidator1(
            String[] schemes, RegexValidator authorityValidator_param, long options_param) {
        return new routines_UrlValidator(
                schemes,
                authorityValidator_param,
                options_param,
                DomainValidator.getInstance1(isOn1(ALLOW_LOCAL_URLS, options_param)));
    }

    public static routines_UrlValidator UrlValidator2(RegexValidator authorityValidator_param, long options_param) {
        return UrlValidator1(null, authorityValidator_param, options_param);
    }

    public static routines_UrlValidator UrlValidator3(String[] schemes, long options_param) {
        return UrlValidator1(schemes, null, options_param);
    }

    public static routines_UrlValidator UrlValidator4(long options_param) {
        return UrlValidator1(null, null, options_param);
    }

    public static routines_UrlValidator UrlValidator5(String[] schemes) {
        return UrlValidator3(schemes, 0L);
    }

    public static routines_UrlValidator UrlValidator6() {
    ...
    /**
     * Initialize a routines_UrlValidator with the given validation options.
     *
     ...
     * @return whether the specified flag value is on.
     */
    private static boolean isOn1(long flag, long options_param) {
        return (options_param & flag) > 0;
    }
