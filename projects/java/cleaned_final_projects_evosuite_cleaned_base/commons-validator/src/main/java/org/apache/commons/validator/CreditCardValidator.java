 ...
 * </pre>
 *
 * <p>configures the validator to only pass American Express and CreditCardValidator_Visa cards. If a card type is not
 * directly supported by this class, you can implement the CreditCardValidator_CreditCardType interface and pass an
...
public class CreditCardValidator {
    ...
    /** Option specifying that CreditCardValidator_Visa cards are allowed. */
    public static final int VISA = 1 << 1;

    /** Option specifying that CreditCardValidator_Mastercard cards are allowed. */
    public static final int MASTERCARD = 1 << 2;

    /** Option specifying that CreditCardValidator_Discover cards are allowed. */
    public static final int DISCOVER = 1 << 3;

    /** The CreditCardTypes that are allowed to pass validation. */
    private final Collection<CreditCardValidator_CreditCardType> cardTypes = new ArrayList<CreditCardValidator_CreditCardType>();
    ...
    public CreditCardValidator(int options) {
        ...
        if (f.isOn(VISA)) {
            this.cardTypes.add(new CreditCardValidator_Visa());
        }

        if (f.isOn(AMEX)) {
            this.cardTypes.add(new CreditCardValidator_Amex());
        }

        if (f.isOn(MASTERCARD)) {
            this.cardTypes.add(new CreditCardValidator_Mastercard());
        }

        if (f.isOn(DISCOVER)) {
            this.cardTypes.add(new CreditCardValidator_Discover());
        }
    }
    ...
    public boolean isValid(String card) {
        ...
        for (Object cardType : this.cardTypes) {
            CreditCardValidator_CreditCardType type__ = (CreditCardValidator_CreditCardType) cardType;
            if (type__.matches(card)) {
                return true;
            }
        }
    ...
    /**
     * Adds an allowed CreditCardValidator_CreditCardType that participates in the card validation algorithm.
     *
     * @param type The type that is now allowed to pass validation.
     * @since Validator 1.1.2
     */
    public void addAllowedCardType(CreditCardValidator_CreditCardType type__) {
        this.cardTypes.add(type__);
    }
    ...
    /**
     * CreditCardValidator_CreditCardType implementations define how validation is performed for one type/brand of
     * credit card.
     *
     * @since Validator 1.1.2
     */
    public interface CreditCardValidator_CreditCardType {
    ...
    /** Change to support CreditCardValidator_Visa Carte Blue used in France has been removed - see Bug 35926 */
    private static class CreditCardValidator_Visa implements CreditCardValidator_CreditCardType {
    ...
    private static class CreditCardValidator_Amex implements CreditCardValidator_CreditCardType {
    ...
    private static class CreditCardValidator_Discover implements CreditCardValidator_CreditCardType {
    ...
    private static class CreditCardValidator_Mastercard implements CreditCardValidator_CreditCardType {
