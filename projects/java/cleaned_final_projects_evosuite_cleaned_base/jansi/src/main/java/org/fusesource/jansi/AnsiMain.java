...
public class AnsiMain {
    ...
    private static void diagnoseTty(boolean stderr) {
        ...
        if (AnsiConsole.IS_WINDOWS) {
            ...
            if ((AnsiConsole.IS_CONEMU || AnsiConsole.IS_CYGWIN || AnsiConsole.IS_MSYSTEM) && isatty == 0) {
                ...
                Kernel32.Kernel32_CONSOLE_SCREEN_BUFFER_INFO info = new Kernel32.Kernel32_CONSOLE_SCREEN_BUFFER_INFO();
            ...
            CLibrary.CLibrary_WinSize ws = new CLibrary.CLibrary_WinSize((short) 0, (short) 0);
    ...
    private static String getPomPropertiesVersion(String path) throws IOException {
        InputStream in__ = AnsiMain.class.getResourceAsStream("/META-INF/maven/" + path + "/pom.properties");
        if (in__ == null) {
            return null;
        }
        try {
    ...
    private static void writeFileContent(File f) throws IOException {
        InputStream in__ = new FileInputStream(f);
        try {
            byte[] buf = new byte[1024];
            int l = 0;
            while ((l = in__.read(buf)) >= 0) {
