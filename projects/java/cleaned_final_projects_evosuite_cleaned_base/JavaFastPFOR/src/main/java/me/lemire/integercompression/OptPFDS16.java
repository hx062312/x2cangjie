...
public final class OptPFDS16 implements IntegerCODEC,SkippableIntegerCODEC {
        ...
        public void headlessCompress(int[] in__, IntWrapper inpos, int inlength,
                ...
                encodePage(in__, inpos, inlength, out, outpos);
        ...
        private void getBestBFromData(int[] in__, int pos, IntWrapper bestb,
                IntWrapper bestexcept) {
                final int mb = Util.maxbits(in__, pos, BLOCK_SIZE);
                int mini = 0;
        ...
        private void encodePage(int[] in__, IntWrapper inpos, int thissize,
                ...
                for (final int finalinpos = tmpinpos + thissize; tmpinpos
                        ...
                        getBestBFromData(in__, tmpinpos, bestb, bestexcept);
                        ...
                        if (nbrexcept > 0) {
                                ...
                                for (int i = 0; i < BLOCK_SIZE; ++i) {
                                        if ((in__[tmpinpos + i] >>> bits[tmpbestb]) != 0) {
                                                exceptbuffer[c + nbrexcept] = i;
                                                exceptbuffer[c] = in__[tmpinpos
                                                        + i] >>> bits[tmpbestb];
                                                ++c;
                                        }
                                }
        ...
        public void headlessUncompress(int[] in__, IntWrapper inpos, int inlength,
                ...
                decodePage(in__, inpos, out, outpos, mynvalue);
        }
        ...
        private void decodePage(int[] in__, IntWrapper inpos, int[] out,
                ...
                for (int run = 0; run < thissize / BLOCK_SIZE; ++run, tmpoutpos += BLOCK_SIZE) {
                        final int b = in__[tmpinpos] & 0xFF;
                        final int cexcept = (in__[tmpinpos] >>> 8) & 0xFF;
                        final int exceptsize = (in__[tmpinpos] >>> 16);
                        ++tmpinpos;
                        S16.uncompress(in__, tmpinpos, exceptsize, exceptbuffer,
                                0, 2 * cexcept);
                        tmpinpos += exceptsize;
                        for (int k = 0; k < BLOCK_SIZE; k += 32) {
