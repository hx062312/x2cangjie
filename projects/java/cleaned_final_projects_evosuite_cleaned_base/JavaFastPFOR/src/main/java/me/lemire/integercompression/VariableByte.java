...
public class VariableByte implements IntegerCODEC, ByteIntegerCODEC, SkippableIntegerCODEC {
    ...
    @Override
    public void headlessCompress(int[] in__, IntWrapper inpos, int inlength, int[] out,
            IntWrapper outpos) {
        if (inlength == 0)
            return;
        ...
        for (int k = inpos.get(); k < inpos.get() + inlength; ++k) {
            final long val = in__[k] & 0xFFFFFFFFL; // To be consistent with
    ...
    @Override
    public void compress1(int[] in__, IntWrapper inpos, int inlength, byte[] out,
                          IntWrapper outpos) {
        if (inlength == 0)
            return;
    ...
    @Override
    public void headlessUncompress(int[] in__, IntWrapper inpos, int inlength, int[] out,
