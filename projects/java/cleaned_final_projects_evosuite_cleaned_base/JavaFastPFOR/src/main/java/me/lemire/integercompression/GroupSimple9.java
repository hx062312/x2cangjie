...
public final class GroupSimple9 implements IntegerCODEC, SkippableIntegerCODEC {
	...
	@Override
	public void compress0(int[] in__, IntWrapper inpos, int inlength, int out[], IntWrapper outpos) {
		if (inlength == 0)
			return;
	...
	private void encode0(final int[] in__, final int inf, final int code, final int[] out,
			final int outf) {
		for (int i = 0; i < 24; i++)
			out[outf + 0] = (out[outf + 0] << 1) + (in__[inf + i]);
		for (int i = 0; i < 4; i++)
			out[outf + 1] = (out[outf + 1] << 1) + in__[inf + 24 + i];
		for (int i = 0; i < 28; i++)
			out[outf + 1] = (out[outf + 1] << 1) + in__[inf + 28 + i];
		out[outf + 0] = code << 24 | out[outf + 0];
		
	}

	private void encode1(final int[] in__, final int inf, final int code, final int[] out,
			final int outf) {
		for (int i = 0; i < 24; i++)
			out[outf + 0] = (out[outf + 0] << 1) + in__[inf + i];
		for (int i = 0; i < 4; i++)
			out[outf + 1] = (out[outf + 1] << 1) + in__[inf + 24 + i];
		for (int i = 0; i < 14; i++)
			out[outf + 1] = (out[outf + 1] << 2) + in__[inf + 28 + i];
		out[outf + 0] = code << 24 | out[outf + 0];
		
	}

	private void encode2(final int[] in__, final int inf, final int code, final int[] out,
			final int outf) {
		for (int i = 0; i < 24; i++)
			out[outf + 0] = (out[outf + 0] << 1) + in__[inf + i];
		for (int i = 0; i < 4; i++)
			out[outf + 1] = (out[outf + 1] << 1) + in__[inf + 24 + i];
		for (int i = 0; i < 9; i++)
			out[outf + 1] = (out[outf + 1] << 3) + in__[inf + 28 + i];// 第二个28位是低位存储的，所以浪费的1比特在最顶端。
		out[outf + 0] = code << 24 | out[outf + 0];
		
	}

	private void encode3(final int[] in__, final int inf, final int code, final int[] out,
			final int outf) {
		for (int i = 0; i < 24; i++)
			out[outf + 0] = (out[outf + 0] << 1) + in__[inf + i];
		for (int i = 0; i < 4; i++)
			out[outf + 1] = (out[outf + 1] << 1) + in__[inf + 24 + i];
		for (int i = 0; i < 7; i++)
			out[outf + 1] = (out[outf + 1] << 4) + in__[inf + 28 + i];
		out[outf + 0] = code << 24 | out[outf + 0];
		
	}

	private void encode4(final int[] in__, final int inf, final int code, final int[] out,
			final int outf) {
		for (int i = 0; i < 24; i++)
			out[outf + 0] = (out[outf + 0] << 1) + in__[inf + i];
		for (int i = 0; i < 4; i++)
			out[outf + 1] = (out[outf + 1] << 1) + in__[inf + 24 + i];
		for (int i = 0; i < 5; i++)
			out[outf + 1] = (out[outf + 1] << 5) + in__[inf + 28 + i];
		out[outf + 0] = code << 24 | out[outf + 0];
		
	}

	private void encode5(final int[] in__, final int inf, final int code, final int[] out,
			final int outf) {
		for (int i = 0; i < 24; i++)
			out[outf + 0] = (out[outf + 0] << 1) + in__[inf + i];
		for (int i = 0; i < 4; i++)
			out[outf + 1] = (out[outf + 1] << 1) + in__[inf + 24 + i];
		for (int i = 0; i < 4; i++)
			out[outf + 1] = (out[outf + 1] << 7) + in__[inf + 28 + i];
		out[outf + 0] = code << 24 | out[outf + 0];
		
	}

	private void encode6(final int[] in__, final int inf, final int code, final int[] out,
			final int outf) {
		for (int i = 0; i < 24; i++)
			out[outf + 0] = (out[outf + 0] << 1) + in__[inf + i];
		for (int i = 0; i < 4; i++)
			out[outf + 1] = (out[outf + 1] << 1) + in__[inf + 24 + i];
		for (int i = 0; i < 3; i++)
			out[outf + 1] = (out[outf + 1] << 9) + in__[inf + 28 + i];
		out[outf + 0] = code << 24 | out[outf + 0];
		
	}

	private void encode7(final int[] in__, final int inf, final int code, final int[] out,
			final int outf) {
		for (int i = 0; i < 24; i++)
			out[outf + 0] = (out[outf + 0] << 1) + in__[inf + i];
		for (int i = 0; i < 4; i++)
			out[outf + 1] = (out[outf + 1] << 1) + in__[inf + 24 + i];
		for (int i = 0; i < 2; i++)
			out[outf + 1] = (out[outf + 1] << 14) + in__[inf + 28 + i];
		out[outf + 0] = code << 24 | out[outf + 0];
		
	}

	private void encode8(final int[] in__, final int inf, final int code, final int[] out,
			final int outf) {
		for (int i = 0; i < 24; i++)
			out[outf + 0] = (out[outf + 0] << 1) + in__[inf + i];
		for (int i = 0; i < 4; i++)
			out[outf + 1] = (out[outf + 1] << 1) + in__[inf + 24 + i];
		for (int i = 0; i < 1; i++)
			out[outf + 1] = (out[outf + 1] << 28) + in__[inf + 28 + i];
		out[outf + 0] = code << 24 | out[outf + 0];
		
	}

	private void encode9(final int[] in__, final int inf, final int code, final int[] out,
			final int outf) {
		for (int i = 0; i < 12; i++)
			out[outf + 0] = (out[outf + 0] << 2) + in__[inf + i];
		for (int i = 0; i < 2; i++)
			out[outf + 1] = (out[outf + 1] << 2) + in__[inf + 12 + i];
		for (int i = 0; i < 28; i++)
			out[outf + 1] = (out[outf + 1] << 1) + in__[inf + 14 + i];

		out[outf + 0] = code << 24 | out[outf + 0];
		
	}

	private void encode10(final int[] in__, final int inf, final int code, final int[] out,
			final int outf) {
		for (int i = 0; i < 12; i++) {
			out[outf + 0] = (out[outf + 0] << 2) + in__[inf + i];
		...
		for (int i = 0; i < 2; i++)
			out[outf + 1] = (out[outf + 1] << 2) + in__[inf + 12 + i];
		for (int i = 0; i < 14; i++)
			out[outf + 1] = (out[outf + 1] << 2) + in__[inf + 14 + i];

		out[outf + 0] = code << 24 | out[outf + 0];
		
	}

	private void encode11(final int[] in__, final int inf, final int code, final int[] out,
			final int outf) {
		for (int i = 0; i < 12; i++)
			out[outf + 0] = (out[outf + 0] << 2) + in__[inf + i];
		for (int i = 0; i < 2; i++)
			out[outf + 1] = (out[outf + 1] << 2) + in__[inf + 12 + i];
		for (int i = 0; i < 9; i++)
			out[outf + 1] = (out[outf + 1] << 3) + in__[inf + 14 + i];

		out[outf + 0] = code << 24 | out[outf + 0];
		
	}

	private void encode12(final int[] in__, final int inf, final int code, final int[] out,
			final int outf) {
		for (int i = 0; i < 12; i++)
			out[outf + 0] = (out[outf + 0] << 2) + in__[inf + i];
		for (int i = 0; i < 2; i++)
			out[outf + 1] = (out[outf + 1] << 2) + in__[inf + 12 + i];
		for (int i = 0; i < 7; i++)
			out[outf + 1] = (out[outf + 1] << 4) + in__[inf + 14 + i];

		out[outf + 0] = code << 24 | out[outf + 0];
		
	}

	private void encode13(final int[] in__, final int inf, final int code, final int[] out,
			final int outf) {
		for (int i = 0; i < 12; i++)
			out[outf + 0] = (out[outf + 0] << 2) + in__[inf + i];
		for (int i = 0; i < 2; i++)
			out[outf + 1] = (out[outf + 1] << 2) + in__[inf + 12 + i];
		for (int i = 0; i < 5; i++)
			out[outf + 1] = (out[outf + 1] << 5) + in__[inf + 14 + i];

		out[outf + 0] = code << 24 | out[outf + 0];
		
	}

	private void encode14(final int[] in__, final int inf, final int code, final int[] out,
			final int outf) {
		for (int i = 0; i < 12; i++)
			out[outf + 0] = (out[outf + 0] << 2) + in__[inf + i];
		for (int i = 0; i < 2; i++)
			out[outf + 1] = (out[outf + 1] << 2) + in__[inf + 12 + i];
		for (int i = 0; i < 4; i++)
			out[outf + 1] = (out[outf + 1] << 7) + in__[inf + 14 + i];

		out[outf + 0] = code << 24 | out[outf + 0];
		
	}

	private void encode15(final int[] in__, final int inf, final int code, final int[] out,
			final int outf) {
		for (int i = 0; i < 12; i++)
			out[outf + 0] = (out[outf + 0] << 2) + in__[inf + i];
		for (int i = 0; i < 2; i++)
			out[outf + 1] = (out[outf + 1] << 2) + in__[inf + 12 + i];
		for (int i = 0; i < 3; i++)
			out[outf + 1] = (out[outf + 1] << 9) + in__[inf + 14 + i];

		out[outf + 0] = code << 24 | out[outf + 0];
		
	}

	private void encode16(final int[] in__, final int inf, final int code, final int[] out,
			final int outf) {
		for (int i = 0; i < 12; i++)
			out[outf + 0] = (out[outf + 0] << 2) + in__[inf + i];
		for (int i = 0; i < 2; i++)
			out[outf + 1] = (out[outf + 1] << 2) + in__[inf + 12 + i];
		for (int i = 0; i < 2; i++)
			out[outf + 1] = (out[outf + 1] << 14) + in__[inf + 14 + i];

		out[outf + 0] = code << 24 | out[outf + 0];
		
	}

	private void encode17(final int[] in__, final int inf, final int code, final int[] out,
			final int outf) {
		for (int i = 0; i < 12; i++)
			out[outf + 0] = (out[outf + 0] << 2) + in__[inf + i];
		for (int i = 0; i < 2; i++)
			out[outf + 1] = (out[outf + 1] << 2) + in__[inf + 12 + i];
		for (int i = 0; i < 1; i++)
			out[outf + 1] = (out[outf + 1] << 28) + in__[inf + 14 + i];

		out[outf + 0] = code << 24 | out[outf + 0];
		
	}

	private void encode18(final int[] in__, final int inf, final int code, final int[] out,
			final int outf) {
		for (int i = 0; i < 8; i++)
			out[outf + 0] = (out[outf + 0] << 3) + in__[inf + i];
		for (int i = 0; i < 1; i++)
			out[outf + 1] = (out[outf + 1] << 3) + in__[inf + 8 + i];
		for (int i = 0; i < 28; i++)
			out[outf + 1] = (out[outf + 1] << 1) + in__[inf + 9 + i];

		out[outf + 0] = code << 24 | out[outf + 0];
		
	}

	private void encode19(final int[] in__, final int inf, final int code, final int[] out,
			final int outf) {
		for (int i = 0; i < 8; i++)
			out[outf + 0] = (out[outf + 0] << 3) + in__[inf + i];
		for (int i = 0; i < 1; i++)
			out[outf + 1] = (out[outf + 1] << 3) + in__[inf + 8 + i];
		for (int i = 0; i < 14; i++)
			out[outf + 1] = (out[outf + 1] << 2) + in__[inf + 9 + i];

		out[outf + 0] = code << 24 | out[outf + 0];
		
	}

	private void encode20(final int[] in__, final int inf, final int code, final int[] out,
			final int outf) {
		for (int i = 0; i < 8; i++)
			out[outf + 0] = (out[outf + 0] << 3) + in__[inf + i];
		for (int i = 0; i < 1; i++)
			out[outf + 1] = (out[outf + 1] << 3) + in__[inf + 8 + i];
		for (int i = 0; i < 9; i++)
			out[outf + 1] = (out[outf + 1] << 3) + in__[inf + 9 + i];

		out[outf + 0] = code << 24 | out[outf + 0];
		
	}

	private void encode21(final int[] in__, final int inf, final int code, final int[] out,
			final int outf) {
		for (int i = 0; i < 8; i++)
			out[outf + 0] = (out[outf + 0] << 3) + in__[inf + i];
		for (int i = 0; i < 1; i++)
			out[outf + 1] = (out[outf + 1] << 3) + in__[inf + 8 + i];
		for (int i = 0; i < 7; i++)
			out[outf + 1] = (out[outf + 1] << 4) + in__[inf + 9 + i];
		out[outf + 0] = code << 24 | out[outf + 0];
		
	}

	private void encode22(final int[] in__, final int inf, final int code, final int[] out,
			final int outf) {
		for (int i = 0; i < 8; i++)
			out[outf + 0] = (out[outf + 0] << 3) + in__[inf + i];
		for (int i = 0; i < 1; i++)
			out[outf + 1] = (out[outf + 1] << 3) + in__[inf + 8 + i];
		for (int i = 0; i < 5; i++)
			out[outf + 1] = (out[outf + 1] << 5) + in__[inf + 9 + i];
		out[outf + 0] = code << 24 | out[outf + 0];
		
	}

	private void encode23(final int[] in__, final int inf, final int code, final int[] out,
			final int outf) {
		for (int i = 0; i < 8; i++)
			out[outf + 0] = (out[outf + 0] << 3) + in__[inf + i];
		for (int i = 0; i < 1; i++)
			out[outf + 1] = (out[outf + 1] << 3) + in__[inf + 8 + i];
		for (int i = 0; i < 4; i++)
			out[outf + 1] = (out[outf + 1] << 7) + in__[inf + 9 + i];
		out[outf + 0] = code << 24 | out[outf + 0];
		
	}

	private void encode24(final int[] in__, final int inf, final int code, final int[] out,
			final int outf) {
		for (int i = 0; i < 8; i++)
			out[outf + 0] = (out[outf + 0] << 3) + in__[inf + i];
		for (int i = 0; i < 1; i++)
			out[outf + 1] = (out[outf + 1] << 3) + in__[inf + 8 + i];
		for (int i = 0; i < 3; i++)
			out[outf + 1] = (out[outf + 1] << 9) + in__[inf + 9 + i];
		out[outf + 0] = code << 24 | out[outf + 0];
		
	}

	private void encode25(final int[] in__, final int inf, final int code, final int[] out,
			final int outf) {
		for (int i = 0; i < 8; i++)
			out[outf + 0] = (out[outf + 0] << 3) + in__[inf + i];
		for (int i = 0; i < 1; i++)
			out[outf + 1] = (out[outf + 1] << 3) + in__[inf + 8 + i];
		for (int i = 0; i < 2; i++)
			out[outf + 1] = (out[outf + 1] << 14) + in__[inf + 9 + i];
		out[outf + 0] = code << 24 | out[outf + 0];
		
	}

	private void encode26(final int[] in__, final int inf, final int code, final int[] out,
			final int outf) {
		for (int i = 0; i < 8; i++)
			out[outf + 0] = (out[outf + 0] << 3) + in__[inf + i];
		for (int i = 0; i < 1; i++)
			out[outf + 1] = (out[outf + 1] << 3) + in__[inf + 8 + i];
		for (int i = 0; i < 1; i++)
			out[outf + 1] = (out[outf + 1] << 28) + in__[inf + 9 + i];
		out[outf + 0] = code << 24 | out[outf + 0];
		
	}

	private void encode27(final int[] in__, final int inf, final int code, final int[] out,
			final int outf) {
		for (int i = 0; i < 6; i++)
			out[outf + 0] = (out[outf + 0] << 4) + in__[inf + i];
		for (int i = 0; i < 1; i++)
			out[outf + 1] = (out[outf + 1] << 4) + in__[inf + 6 + i];
		for (int i = 0; i < 28; i++)
			out[outf + 1] = (out[outf + 1] << 1) + in__[inf + 7 + i];
		out[outf + 0] = code << 24 | out[outf + 0];
		
	}

	private void encode28(final int[] in__, final int inf, final int code, final int[] out,
			final int outf) {
		for (int i = 0; i < 6; i++)
			out[outf + 0] = (out[outf + 0] << 4) + in__[inf + i];
		for (int i = 0; i < 1; i++)
			out[outf + 1] = (out[outf + 1] << 4) + in__[inf + 6 + i];
		for (int i = 0; i < 14; i++)
			out[outf + 1] = (out[outf + 1] << 2) + in__[inf + 7 + i];
		out[outf + 0] = code << 24 | out[outf + 0];
		
	}

	private void encode29(final int[] in__, final int inf, final int code, final int[] out,
			final int outf) {
		for (int i = 0; i < 6; i++)
			out[outf + 0] = (out[outf + 0] << 4) + in__[inf + i];
		for (int i = 0; i < 1; i++)
			out[outf + 1] = (out[outf + 1] << 4) + in__[inf + 6 + i];
		for (int i = 0; i < 9; i++)
			out[outf + 1] = (out[outf + 1] << 3) + in__[inf + 7 + i];
		out[outf + 0] = code << 24 | out[outf + 0];
		
	}

	private void encode30(final int[] in__, final int inf, final int code, final int[] out,
			final int outf) {
		for (int i = 0; i < 6; i++)
			out[outf + 0] = (out[outf + 0] << 4) + in__[inf + i];
		for (int i = 0; i < 1; i++)
			out[outf + 1] = (out[outf + 1] << 4) + in__[inf + 6 + i];
		for (int i = 0; i < 7; i++)
			out[outf + 1] = (out[outf + 1] << 4) + in__[inf + 7 + i];
		out[outf + 0] = code << 24 | out[outf + 0];
		
	}

	private void encode31(final int[] in__, final int inf, final int code, final int[] out,
			final int outf) {
		for (int i = 0; i < 6; i++)
			out[outf + 0] = (out[outf + 0] << 4) + in__[inf + i];
		for (int i = 0; i < 1; i++)
			out[outf + 1] = (out[outf + 1] << 4) + in__[inf + 6 + i];
		for (int i = 0; i < 5; i++)
			out[outf + 1] = (out[outf + 1] << 5) + in__[inf + 7 + i];
		out[outf + 0] = code << 24 | out[outf + 0];
		
	}

	private void encode32(final int[] in__, final int inf, final int code, final int[] out,
			final int outf) {
		for (int i = 0; i < 6; i++)
			out[outf + 0] = (out[outf + 0] << 4) + in__[inf + i];
		for (int i = 0; i < 1; i++)
			out[outf + 1] = (out[outf + 1] << 4) + in__[inf + 6 + i];
		for (int i = 0; i < 4; i++)
			out[outf + 1] = (out[outf + 1] << 7) + in__[inf + 7 + i];
		out[outf + 0] = code << 24 | out[outf + 0];
		
	}

	private void encode33(final int[] in__, final int inf, final int code, final int[] out,
			final int outf) {
		for (int i = 0; i < 6; i++)
			out[outf + 0] = (out[outf + 0] << 4) + in__[inf + i];
		for (int i = 0; i < 1; i++)
			out[outf + 1] = (out[outf + 1] << 4) + in__[inf + 6 + i];
		for (int i = 0; i < 3; i++)
			out[outf + 1] = (out[outf + 1] << 9) + in__[inf + 7 + i];
		out[outf + 0] = code << 24 | out[outf + 0];
		
	}

	private void encode34(final int[] in__, final int inf, final int code, final int[] out,
			final int outf) {
		for (int i = 0; i < 6; i++)
			out[outf + 0] = (out[outf + 0] << 4) + in__[inf + i];
		for (int i = 0; i < 1; i++)
			out[outf + 1] = (out[outf + 1] << 4) + in__[inf + 6 + i];
		for (int i = 0; i < 2; i++)
			out[outf + 1] = (out[outf + 1] << 14) + in__[inf + 7 + i];
		out[outf + 0] = code << 24 | out[outf + 0];
		
	}

	private void encode35(final int[] in__, final int inf, final int code, final int[] out,
			final int outf) {
		for (int i = 0; i < 6; i++)
			out[outf + 0] = (out[outf + 0] << 4) + in__[inf + i];
		for (int i = 0; i < 1; i++)
			out[outf + 1] = (out[outf + 1] << 4) + in__[inf + 6 + i];
		for (int i = 0; i < 1; i++)
			out[outf + 1] = (out[outf + 1] << 28) + in__[inf + 7 + i];
		out[outf + 0] = code << 24 | out[outf + 0];
		
	}

	private void encode36(final int[] in__, final int inf, final int code, final int[] out,
			final int outf) {
		for (int i = 0; i < 4; i++)
			out[outf + 0] = (out[outf + 0] << 5) + in__[inf + i];
		out[outf + 0] = (out[outf + 0] << 4) + (in__[inf + 4] >>> 1);
		out[outf + 1] = (out[outf + 1] << 1) + ((in__[inf + 4] << 31) >>> 31);
		for (int i = 0; i < 28; i++)
			out[outf + 1] = (out[outf + 1] << 1) + in__[inf + 5 + i];
		out[outf + 0] = code << 24 | out[outf + 0];
		
	}

	private void encode37(final int[] in__, final int inf, final int code, final int[] out,
			final int outf) {
		for (int i = 0; i < 4; i++)
			out[outf + 0] = (out[outf + 0] << 5) + in__[inf + i];
		out[outf + 0] = (out[outf + 0] << 4) + (in__[inf + 4] >>> 1);
		out[outf + 1] = (out[outf + 1] << 1) + ((in__[inf + 4] << 31) >>> 31);
		for (int i = 0; i < 14; i++)
			out[outf + 1] = (out[outf + 1] << 2) + in__[inf + 5 + i];
		out[outf + 0] = code << 24 | out[outf + 0];
		
	}

	private void encode38(final int[] in__, final int inf, final int code, final int[] out,
			final int outf) {
		for (int i = 0; i < 4; i++)
			out[outf + 0] = (out[outf + 0] << 5) + in__[inf + i];
		out[outf + 0] = (out[outf + 0] << 4) + (in__[inf + 4] >>> 1);
		out[outf + 1] = (out[outf + 1] << 1) + ((in__[inf + 4] << 31) >>> 31);
		for (int i = 0; i < 9; i++)
			out[outf + 1] = (out[outf + 1] << 3) + in__[inf + 5 + i];
		out[outf + 0] = code << 24 | out[outf + 0];
		
	}

	private void encode39(final int[] in__, final int inf, final int code, final int[] out,
			final int outf) {
		for (int i = 0; i < 4; i++)
			out[outf + 0] = (out[outf + 0] << 5) + in__[inf + i];
		out[outf + 0] = (out[outf + 0] << 4) + (in__[inf + 4] >>> 1);
		out[outf + 1] = (out[outf + 1] << 1) + ((in__[inf + 4] << 31) >>> 31);
		for (int i = 0; i < 7; i++)
			out[outf + 1] = (out[outf + 1] << 4) + in__[inf + 5 + i];
		out[outf + 0] = code << 24 | out[outf + 0];
		
	}

	private void encode40(final int[] in__, final int inf, final int code, final int[] out,
			final int outf) {
		for (int i = 0; i < 4; i++)
			out[outf + 0] = (out[outf + 0] << 5) + in__[inf + i];
		out[outf + 0] = (out[outf + 0] << 4) + (in__[inf + 4] >>> 1);
		out[outf + 1] = (out[outf + 1] << 1) + ((in__[inf + 4] << 31) >>> 31);
		for (int i = 0; i < 5; i++)
			out[outf + 1] = (out[outf + 1] << 5) + in__[inf + 5 + i];
		out[outf + 0] = code << 24 | out[outf + 0];
		
	}

	private void encode41(final int[] in__, final int inf, final int code, final int[] out,
			final int outf) {
		for (int i = 0; i < 4; i++)
			out[outf + 0] = (out[outf + 0] << 5) + in__[inf + i];
		out[outf + 0] = (out[outf + 0] << 4) + (in__[inf + 4] >>> 1);
		out[outf + 1] = (out[outf + 1] << 1) + ((in__[inf + 4] << 31) >>> 31);
		for (int i = 0; i < 4; i++)
			out[outf + 1] = (out[outf + 1] << 7) + in__[inf + 5 + i];
		out[outf + 0] = code << 24 | out[outf + 0];
		
	}

	private void encode42(final int[] in__, final int inf, final int code, final int[] out,
			final int outf) {
		for (int i = 0; i < 4; i++)
			out[outf + 0] = (out[outf + 0] << 5) + in__[inf + i];
		out[outf + 0] = (out[outf + 0] << 4) + (in__[inf + 4] >>> 1);
		out[outf + 1] = (out[outf + 1] << 1) + ((in__[inf + 4] << 31) >>> 31);
		for (int i = 0; i < 3; i++)
			out[outf + 1] = (out[outf + 1] << 9) + in__[inf + 5 + i];
		out[outf + 0] = code << 24 | out[outf + 0];
		
	}

	private void encode43(final int[] in__, final int inf, final int code, final int[] out,
			final int outf) {
		for (int i = 0; i < 4; i++)
			out[outf + 0] = (out[outf + 0] << 5) + in__[inf + i];
		out[outf + 0] = (out[outf + 0] << 4) + (in__[inf + 4] >>> 1);
		out[outf + 1] = (out[outf + 1] << 1) + ((in__[inf + 4] << 31) >>> 31);
		for (int i = 0; i < 2; i++)
			out[outf + 1] = (out[outf + 1] << 14) + in__[inf + 5 + i];
		out[outf + 0] = code << 24 | out[outf + 0];
		
	}

	private void encode44(final int[] in__, final int inf, final int code, final int[] out,
			final int outf) {
		for (int i = 0; i < 4; i++)
			out[outf + 0] = (out[outf + 0] << 5) + in__[inf + i];
		out[outf + 0] = (out[outf + 0] << 4) + (in__[inf + 4] >>> 1);
		out[outf + 1] = (out[outf + 1] << 1) + ((in__[inf + 4] << 31) >>> 31);
		for (int i = 0; i < 1; i++)
			out[outf + 1] = (out[outf + 1] << 28) + in__[inf + 5 + i];
		out[outf + 0] = code << 24 | out[outf + 0];
		
	}

	private void encode45(final int[] in__, final int inf, final int code, final int[] out,
			final int outf) {
		for (int i = 0; i < 3; i++)
			out[outf + 0] = (out[outf + 0] << 7) + in__[inf + i];
		out[outf + 0] = (out[outf + 0] << 3) + (in__[inf + 3] >>> 4);
		out[outf + 1] = (out[outf + 1] << 4) + ((in__[inf + 3] << 28) >>> 28);
		for (int i = 0; i < 28; i++)
			out[outf + 1] = (out[outf + 1] << 1) + in__[inf + 4 + i];
		out[outf + 0] = code << 24 | out[outf + 0];
		
	}

	private void encode46(final int[] in__, final int inf, final int code, final int[] out,
			final int outf) {
		for (int i = 0; i < 3; i++)
			out[outf + 0] = (out[outf + 0] << 7) + in__[inf + i];
		out[outf + 0] = (out[outf + 0] << 3) + (in__[inf + 3] >>> 4);
		out[outf + 1] = (out[outf + 1] << 4) + ((in__[inf + 3] << 28) >>> 28);
		for (int i = 0; i < 14; i++)
			out[outf + 1] = (out[outf + 1] << 2) + in__[inf + 4 + i];
		out[outf + 0] = code << 24 | out[outf + 0];
		
	}

	private void encode47(final int[] in__, final int inf, final int code, final int[] out,
			final int outf) {
		for (int i = 0; i < 3; i++)
			out[outf + 0] = (out[outf + 0] << 7) + in__[inf + i];
		out[outf + 0] = (out[outf + 0] << 3) + (in__[inf + 3] >>> 4);
		out[outf + 1] = (out[outf + 1] << 4) + ((in__[inf + 3] << 28) >>> 28);
		for (int i = 0; i < 9; i++)
			out[outf + 1] = (out[outf + 1] << 3) + in__[inf + 4 + i];
		out[outf + 0] = code << 24 | out[outf + 0];
		
	}

	private void encode48(final int[] in__, final int inf, final int code, final int[] out,
			final int outf) {
		for (int i = 0; i < 3; i++)
			out[outf + 0] = (out[outf + 0] << 7) + in__[inf + i];
		out[outf + 0] = (out[outf + 0] << 3) + (in__[inf + 3] >>> 4);
		out[outf + 1] = (out[outf + 1] << 4) + ((in__[inf + 3] << 28) >>> 28);
		for (int i = 0; i < 7; i++)
			out[outf + 1] = (out[outf + 1] << 4) + in__[inf + 4 + i];
		out[outf + 0] = code << 24 | out[outf + 0];
		
	}

	private void encode49(final int[] in__, final int inf, final int code, final int[] out,
			final int outf) {
		for (int i = 0; i < 3; i++)
			out[outf + 0] = (out[outf + 0] << 7) + in__[inf + i];
		out[outf + 0] = (out[outf + 0] << 3) + (in__[inf + 3] >>> 4);
		out[outf + 1] = (out[outf + 1] << 4) + ((in__[inf + 3] << 28) >>> 28);
		for (int i = 0; i < 5; i++)
			out[outf + 1] = (out[outf + 1] << 5) + in__[inf + 4 + i];
		out[outf + 0] = code << 24 | out[outf + 0];
		
	}

	private void encode50(final int[] in__, final int inf, final int code, final int[] out,
			final int outf) {
		for (int i = 0; i < 3; i++)
			out[outf + 0] = (out[outf + 0] << 7) + in__[inf + i];
		out[outf + 0] = (out[outf + 0] << 3) + (in__[inf + 3] >>> 4);
		out[outf + 1] = (out[outf + 1] << 4) + ((in__[inf + 3] << 28) >>> 28);
		for (int i = 0; i < 4; i++)
			out[outf + 1] = (out[outf + 1] << 7) + in__[inf + 4 + i];
		out[outf + 0] = code << 24 | out[outf + 0];
		
	}

	private void encode51(final int[] in__, final int inf, final int code, final int[] out,
			final int outf) {
		for (int i = 0; i < 3; i++)
			out[outf + 0] = (out[outf + 0] << 7) + in__[inf + i];
		out[outf + 0] = (out[outf + 0] << 3) + (in__[inf + 3] >>> 4);
		out[outf + 1] = (out[outf + 1] << 4) + ((in__[inf + 3] << 28) >>> 28);
		for (int i = 0; i < 3; i++)
			out[outf + 1] = (out[outf + 1] << 9) + in__[inf + 4 + i];
		out[outf + 0] = code << 24 | out[outf + 0];
		
	}

	private void encode52(final int[] in__, final int inf, final int code, final int[] out,
			final int outf) {
		for (int i = 0; i < 3; i++)
			out[outf + 0] = (out[outf + 0] << 7) + in__[inf + i];
		out[outf + 0] = (out[outf + 0] << 3) + (in__[inf + 3] >>> 4);
		out[outf + 1] = (out[outf + 1] << 4) + ((in__[inf + 3] << 28) >>> 28);
		for (int i = 0; i < 2; i++)
			out[outf + 1] = (out[outf + 1] << 14) + in__[inf + 4 + i];
		out[outf + 0] = code << 24 | out[outf + 0];
		
	}

	private void encode53(final int[] in__, final int inf, final int code, final int[] out,
			final int outf) {
		for (int i = 0; i < 3; i++)
			out[outf + 0] = (out[outf + 0] << 7) + in__[inf + i];
		out[outf + 0] = (out[outf + 0] << 3) + (in__[inf + 3] >>> 4);
		out[outf + 1] = (out[outf + 1] << 4) + ((in__[inf + 3] << 28) >>> 28);
		for (int i = 0; i < 1; i++)
			out[outf + 1] = (out[outf + 1] << 28) + in__[inf + 4 + i];
		out[outf + 0] = code << 24 | out[outf + 0];
		
	}

	private void encode54(final int[] in__, final int inf, final int code, final int[] out,
			final int outf) {
		for (int i = 0; i < 2; i++)
			out[outf + 0] = (out[outf + 0] << 9) + in__[inf + i];
		out[outf + 0] = (out[outf + 0] << 6) + (in__[inf + 2] >>> 3);
		out[outf + 1] = (out[outf + 1] << 3) + ((in__[inf + 2] << 29) >>> 29);
		for (int i = 0; i < 28; i++)
			out[outf + 1] = (out[outf + 1] << 1) + in__[inf + 3 + i];
		out[outf + 0] = code << 24 | out[outf + 0];
		
	}

	private void encode55(final int[] in__, final int inf, final int code, final int[] out,
			final int outf) {
		for (int i = 0; i < 2; i++)
			out[outf + 0] = (out[outf + 0] << 9) + in__[inf + i];
		out[outf + 0] = (out[outf + 0] << 6) + (in__[inf + 2] >>> 3);
		out[outf + 1] = (out[outf + 1] << 3) + ((in__[inf + 2] << 29) >>> 29);
		for (int i = 0; i < 14; i++)
			out[outf + 1] = (out[outf + 1] << 2) + in__[inf + 3 + i];
		out[outf + 0] = code << 24 | out[outf + 0];
		
	}

	private void encode56(final int[] in__, final int inf, final int code, final int[] out,
			final int outf) {
		for (int i = 0; i < 2; i++)
			out[outf + 0] = (out[outf + 0] << 9) + in__[inf + i];
		out[outf + 0] = (out[outf + 0] << 6) + (in__[inf + 2] >>> 3);
		out[outf + 1] = (out[outf + 1] << 3) + ((in__[inf + 2] << 29) >>> 29);
		for (int i = 0; i < 9; i++)
			out[outf + 1] = (out[outf + 1] << 3) + in__[inf + 3 + i];
		out[outf + 0] = code << 24 | out[outf + 0];
		
	}

	private void encode57(final int[] in__, final int inf, final int code, final int[] out,
			final int outf) {
		for (int i = 0; i < 2; i++)
			out[outf + 0] = (out[outf + 0] << 9) + in__[inf + i];
		out[outf + 0] = (out[outf + 0] << 6) + (in__[inf + 2] >>> 3);
		out[outf + 1] = (out[outf + 1] << 3) + ((in__[inf + 2] << 29) >>> 29);
		for (int i = 0; i < 7; i++)
			out[outf + 1] = (out[outf + 1] << 4) + in__[inf + 3 + i];
		out[outf + 0] = code << 24 | out[outf + 0];
		
	}

	private void encode58(final int[] in__, final int inf, final int code, final int[] out,
			final int outf) {
		for (int i = 0; i < 2; i++)
			out[outf + 0] = (out[outf + 0] << 9) + in__[inf + i];
		out[outf + 0] = (out[outf + 0] << 6) + (in__[inf + 2] >>> 3);
		out[outf + 1] = (out[outf + 1] << 3) + ((in__[inf + 2] << 29) >>> 29);
		for (int i = 0; i < 5; i++)
			out[outf + 1] = (out[outf + 1] << 5) + in__[inf + 3 + i];
		out[outf + 0] = code << 24 | out[outf + 0];
		
	}

	private void encode59(final int[] in__, final int inf, final int code, final int[] out,
			final int outf) {
		for (int i = 0; i < 2; i++)
			out[outf + 0] = (out[outf + 0] << 9) + in__[inf + i];
		out[outf + 0] = (out[outf + 0] << 6) + (in__[inf + 2] >>> 3);
		out[outf + 1] = (out[outf + 1] << 3) + ((in__[inf + 2] << 29) >>> 29);
		for (int i = 0; i < 4; i++)
			out[outf + 1] = (out[outf + 1] << 7) + in__[inf + 3 + i];
		out[outf + 0] = code << 24 | out[outf + 0];
		
	}

	private void encode60(final int[] in__, final int inf, final int code, final int[] out,
			final int outf) {
		for (int i = 0; i < 2; i++)
			out[outf + 0] = (out[outf + 0] << 9) + in__[inf + i];
		out[outf + 0] = (out[outf + 0] << 6) + (in__[inf + 2] >>> 3);
		out[outf + 1] = (out[outf + 1] << 3) + ((in__[inf + 2] << 29) >>> 29);
		for (int i = 0; i < 3; i++)
			out[outf + 1] = (out[outf + 1] << 9) + in__[inf + 3 + i];
		out[outf + 0] = code << 24 | out[outf + 0];
		
	}

	private void encode61(final int[] in__, final int inf, final int code, final int[] out,
			final int outf) {
		for (int i = 0; i < 2; i++)
			out[outf + 0] = (out[outf + 0] << 9) + in__[inf + i];
		out[outf + 0] = (out[outf + 0] << 6) + (in__[inf + 2] >>> 3);
		out[outf + 1] = (out[outf + 1] << 3) + ((in__[inf + 2] << 29) >>> 29);
		for (int i = 0; i < 2; i++)
			out[outf + 1] = (out[outf + 1] << 14) + in__[inf + 3 + i];
		out[outf + 0] = code << 24 | out[outf + 0];
		
	}

	private void encode62(final int[] in__, final int inf, final int code, final int[] out,
			final int outf) {
		for (int i = 0; i < 2; i++)
			out[outf + 0] = (out[outf + 0] << 9) + in__[inf + i];
		out[outf + 0] = (out[outf + 0] << 6) + (in__[inf + 2] >>> 3);
		out[outf + 1] = (out[outf + 1] << 3) + ((in__[inf + 2] << 29) >>> 29);
		for (int i = 0; i < 1; i++)
			out[outf + 1] = (out[outf + 1] << 28) + in__[inf + 3 + i];
		out[outf + 0] = code << 24 | out[outf + 0];
		
	}

	private void encode63(final int[] in__, final int inf, final int code, final int[] out,
			final int outf) {

		out[outf + 0] = (out[outf + 0] << 14) + in__[inf];
		out[outf + 0] = (out[outf + 0] << 10) + (in__[inf + 1] >>> 4);
		out[outf + 1] = (out[outf + 1] << 4) + ((in__[inf + 1] << 28) >>> 28);
		for (int i = 0; i < 28; i++)
			out[outf + 1] = (out[outf + 1] << 1) + in__[inf + 2 + i];
		out[outf + 0] = code << 24 | out[outf + 0];
		
	}

	private void encode64(final int[] in__, final int inf, final int code, final int[] out,
			final int outf) {
		out[outf + 0] = (out[outf + 0] << 14) + in__[inf];
		out[outf + 0] = (out[outf + 0] << 10) + (in__[inf + 1] >>> 4);
		out[outf + 1] = (out[outf + 1] << 4) + ((in__[inf + 1] << 28) >>> 28);
		for (int i = 0; i < 14; i++)
			out[outf + 1] = (out[outf + 1] << 2) + in__[inf + 2 + i];
		out[outf + 0] = code << 24 | out[outf + 0];
		
	}

	private void encode65(final int[] in__, final int inf, final int code, final int[] out,
			final int outf) {
		out[outf + 0] = (out[outf + 0] << 14) + in__[inf];
		out[outf + 0] = (out[outf + 0] << 10) + (in__[inf + 1] >>> 4);
		out[outf + 1] = (out[outf + 1] << 4) + ((in__[inf + 1] << 28) >>> 28);
		for (int i = 0; i < 9; i++)
			out[outf + 1] = (out[outf + 1] << 3) + in__[inf + 2 + i];
		out[outf + 0] = code << 24 | out[outf + 0];
		
	}

	private void encode66(final int[] in__, final int inf, final int code, final int[] out,
			final int outf) {
		out[outf + 0] = (out[outf + 0] << 14) + in__[inf];
		out[outf + 0] = (out[outf + 0] << 10) + (in__[inf + 1] >>> 4);
		out[outf + 1] = (out[outf + 1] << 4) + ((in__[inf + 1] << 28) >>> 28);
		for (int i = 0; i < 7; i++)
			out[outf + 1] = (out[outf + 1] << 4) + in__[inf + 2 + i];
		out[outf + 0] = code << 24 | out[outf + 0];
		
	}

	private void encode67(final int[] in__, final int inf, final int code, final int[] out,
			final int outf) {
		out[outf + 0] = (out[outf + 0] << 14) + in__[inf];
		out[outf + 0] = (out[outf + 0] << 10) + (in__[inf + 1] >>> 4);
		out[outf + 1] = (out[outf + 1] << 4) + ((in__[inf + 1] << 28) >>> 28);
		for (int i = 0; i < 5; i++)
			out[outf + 1] = (out[outf + 1] << 5) + in__[inf + 2 + i];
		out[outf + 0] = code << 24 | out[outf + 0];
		
	}

	private void encode68(final int[] in__, final int inf, final int code, final int[] out,
			final int outf) {
		out[outf + 0] = (out[outf + 0] << 14) + in__[inf];
		out[outf + 0] = (out[outf + 0] << 10) + (in__[inf + 1] >>> 4);
		out[outf + 1] = (out[outf + 1] << 4) + ((in__[inf + 1] << 28) >>> 28);
		for (int i = 0; i < 4; i++)
			out[outf + 1] = (out[outf + 1] << 7) + in__[inf + 2 + i];
		out[outf + 0] = code << 24 | out[outf + 0];
		
	}

	private void encode69(final int[] in__, final int inf, final int code, final int[] out,
			final int outf) {
		out[outf + 0] = (out[outf + 0] << 14) + in__[inf];
		out[outf + 0] = (out[outf + 0] << 10) + (in__[inf + 1] >>> 4);
		out[outf + 1] = (out[outf + 1] << 4) + ((in__[inf + 1] << 28) >>> 28);
		for (int i = 0; i < 3; i++)
			out[outf + 1] = (out[outf + 1] << 9) + in__[inf + 2 + i];
		out[outf + 0] = code << 24 | out[outf + 0];
		
	}

	private void encode70(final int[] in__, final int inf, final int code, final int[] out,
			final int outf) {
		out[outf + 0] = (out[outf + 0] << 14) + in__[inf];
		out[outf + 0] = (out[outf + 0] << 10) + (in__[inf + 1] >>> 4);
		out[outf + 1] = (out[outf + 1] << 4) + ((in__[inf + 1] << 28) >>> 28);
		for (int i = 0; i < 2; i++)
			out[outf + 1] = (out[outf + 1] << 14) + in__[inf + 2 + i];
		out[outf + 0] = code << 24 | out[outf + 0];
		
	}

	private void encode71(final int[] in__, final int inf, final int code, final int[] out,
			final int outf) {
		out[outf + 0] = (out[outf + 0] << 14) + in__[inf];
		out[outf + 0] = (out[outf + 0] << 10) + (in__[inf + 1] >>> 4);
		out[outf + 1] = (out[outf + 1] << 4) + ((in__[inf + 1] << 28) >>> 28);
		for (int i = 0; i < 1; i++)
			out[outf + 1] = (out[outf + 1] << 28) + in__[inf + 2 + i];
		out[outf + 0] = code << 24 | out[outf + 0];
		
	}

	private void encode72(final int[] in__, final int inf, final int code, final int[] out,
			final int outf) {

		out[outf + 0] = (out[outf + 0] << 24) + (in__[inf] >>> 4);
		out[outf + 1] = (out[outf + 1] << 4) + ((in__[inf] << 28) >>> 28);
		for (int i = 0; i < 28; i++)
			out[outf + 1] = (out[outf + 1] << 1) + in__[inf + 1 + i];
		out[outf + 0] = code << 24 | out[outf + 0];
		
	}

	private void encode73(final int[] in__, final int inf, final int code, final int[] out,
			final int outf) {
		out[outf + 0] = (out[outf + 0] << 24) + (in__[inf] >>> 4);
		out[outf + 1] = (out[outf + 1] << 4) + ((in__[inf] << 28) >>> 28);
		for (int i = 0; i < 14; i++)
			out[outf + 1] = (out[outf + 1] << 2) + in__[inf + 1 + i];
		out[outf + 0] = code << 24 | out[outf + 0];
		
	}

	private void encode74(final int[] in__, final int inf, final int code, final int[] out,
			final int outf) {
		out[outf + 0] = (out[outf + 0] << 24) + (in__[inf] >>> 4);
		out[outf + 1] = (out[outf + 1] << 4) + ((in__[inf] << 28) >>> 28);
		for (int i = 0; i < 9; i++)
			out[outf + 1] = (out[outf + 1] << 3) + in__[inf + 1 + i];
		out[outf + 0] = code << 24 | out[outf + 0];
		
	}

	private void encode75(final int[] in__, final int inf, final int code, final int[] out,
			final int outf) {
		out[outf + 0] = (out[outf + 0] << 24) + (in__[inf] >>> 4);
		out[outf + 1] = (out[outf + 1] << 4) + ((in__[inf] << 28) >>> 28);
		for (int i = 0; i < 7; i++)
			out[outf + 1] = (out[outf + 1] << 4) + in__[inf + 1 + i];
		out[outf + 0] = code << 24 | out[outf + 0];
		
	}

	private void encode76(final int[] in__, final int inf, final int code, final int[] out,
			final int outf) {
		out[outf + 0] = (out[outf + 0] << 24) + (in__[inf] >>> 4);
		out[outf + 1] = (out[outf + 1] << 4) + ((in__[inf] << 28) >>> 28);
		for (int i = 0; i < 5; i++)
			out[outf + 1] = (out[outf + 1] << 5) + in__[inf + 1 + i];
		out[outf + 0] = code << 24 | out[outf + 0];
		
	}

	private void encode77(final int[] in__, final int inf, final int code, final int[] out,
			final int outf) {
		out[outf + 0] = (out[outf + 0] << 24) + (in__[inf] >>> 4);
		out[outf + 1] = (out[outf + 1] << 4) + ((in__[inf] << 28) >>> 28);
		for (int i = 0; i < 4; i++)
			out[outf + 1] = (out[outf + 1] << 7) + in__[inf + 1 + i];
		out[outf + 0] = code << 24 | out[outf + 0];
		
	}

	private void encode78(final int[] in__, final int inf, final int code, final int[] out,
			final int outf) {
		out[outf + 0] = (out[outf + 0] << 24) + (in__[inf] >>> 4);
		out[outf + 1] = (out[outf + 1] << 4) + ((in__[inf] << 28) >>> 28);
		for (int i = 0; i < 3; i++)
			out[outf + 1] = (out[outf + 1] << 9) + in__[inf + 1 + i];
		out[outf + 0] = code << 24 | out[outf + 0];
		
	}

	private void encode79(final int[] in__, final int inf, final int code, final int[] out,
			final int outf) {
		out[outf + 0] = (out[outf + 0] << 24) + (in__[inf] >>> 4);
		out[outf + 1] = (out[outf + 1] << 4) + ((in__[inf] << 28) >>> 28);
		for (int i = 0; i < 2; i++)
			out[outf + 1] = (out[outf + 1] << 14) + in__[inf + 1 + i];
		out[outf + 0] = code << 24 | out[outf + 0];
		
	}

	private void encode80(final int[] in__, final int inf, final int code, final int[] out,
			final int outf) {
		out[outf + 0] = (out[outf + 0] << 24) + (in__[inf] >>> 4);
		out[outf + 1] = (out[outf + 1] << 4) + ((in__[inf] << 28) >>> 28);
		for (int i = 0; i < 1; i++)
			out[outf + 1] = (out[outf + 1] << 28) + in__[inf + 1 + i];
		out[outf + 0] = code << 24 | out[outf + 0];
		
	}

	@Override
	public void uncompress0(int[] in__, IntWrapper inpos, int inlength, int[] out, IntWrapper outpos) {
		if (inlength == 0)
			return;
		final int outlength = in__[inpos.get()];
		inpos.increment();
		headlessUncompress(in__, inpos, inlength, out, outpos, outlength);
	}
	...
	@Override
	public void headlessCompress(int[] in__, IntWrapper inpos, int inlength, int[] out, IntWrapper outpos) {
		...
		while (currentPos < finalin - 28 * 2) {
			int nextCurrentPos = currentPos;
				...
				for (; i < compressedNum; i++) {
					if (Util.smallerorequalthan(max, in__[nextCurrentPos + i]))
						continue mainloop1;
				}
			...
			switch (code) {
			case 0:
				encode0(in__, currentPos, code, out, tmpoutpos);
				break;
			case 1:
				encode1(in__, currentPos, code, out, tmpoutpos);
				break;
			case 2:
				encode2(in__, currentPos, code, out, tmpoutpos);
				break;
			case 3:
				encode3(in__, currentPos, code, out, tmpoutpos);
				break;
			case 4:
				encode4(in__, currentPos, code, out, tmpoutpos);
				break;
			case 5:
				encode5(in__, currentPos, code, out, tmpoutpos);
				break;
			case 6:
				encode6(in__, currentPos, code, out, tmpoutpos);
				break;
			case 7:
				encode7(in__, currentPos, code, out, tmpoutpos);
				break;
			case 8:
				encode8(in__, currentPos, code, out, tmpoutpos);
				break;
			case 9:
				encode9(in__, currentPos, code, out, tmpoutpos);
				break;
			case 10:
				encode10(in__, currentPos, code, out, tmpoutpos);
				break;
			case 11:
				encode11(in__, currentPos, code, out, tmpoutpos);
				break;
			case 12:
				encode12(in__, currentPos, code, out, tmpoutpos);
				break;
			case 13:
				encode13(in__, currentPos, code, out, tmpoutpos);
				break;
			case 14:
				encode14(in__, currentPos, code, out, tmpoutpos);
				break;
			case 15:
				encode15(in__, currentPos, code, out, tmpoutpos);
				break;
			case 16:
				encode16(in__, currentPos, code, out, tmpoutpos);
				break;
			case 17:
				encode17(in__, currentPos, code, out, tmpoutpos);
				break;
			case 18:
				encode18(in__, currentPos, code, out, tmpoutpos);
				break;
			case 19:
				encode19(in__, currentPos, code, out, tmpoutpos);
				break;
			case 20:
				encode20(in__, currentPos, code, out, tmpoutpos);
				break;
			case 21:
				encode21(in__, currentPos, code, out, tmpoutpos);
				break;
			case 22:
				encode22(in__, currentPos, code, out, tmpoutpos);
				break;
			case 23:
				encode23(in__, currentPos, code, out, tmpoutpos);
				break;
			case 24:
				encode24(in__, currentPos, code, out, tmpoutpos);
				break;
			case 25:
				encode25(in__, currentPos, code, out, tmpoutpos);
				break;
			case 26:
				encode26(in__, currentPos, code, out, tmpoutpos);
				break;
			case 27:
				encode27(in__, currentPos, code, out, tmpoutpos);
				break;
			case 28:
				encode28(in__, currentPos, code, out, tmpoutpos);
				break;
			case 29:
				encode29(in__, currentPos, code, out, tmpoutpos);
				break;
			case 30:
				encode30(in__, currentPos, code, out, tmpoutpos);
				break;
			case 31:
				encode31(in__, currentPos, code, out, tmpoutpos);
				break;
			case 32:
				encode32(in__, currentPos, code, out, tmpoutpos);
				break;
			case 33:
				encode33(in__, currentPos, code, out, tmpoutpos);
				break;
			case 34:
				encode34(in__, currentPos, code, out, tmpoutpos);
				break;
			case 35:
				encode35(in__, currentPos, code, out, tmpoutpos);
				break;
			case 36:
				encode36(in__, currentPos, code, out, tmpoutpos);
				break;
			case 37:
				encode37(in__, currentPos, code, out, tmpoutpos);
				break;
			case 38:
				encode38(in__, currentPos, code, out, tmpoutpos);
				break;
			case 39:
				encode39(in__, currentPos, code, out, tmpoutpos);
				break;
			case 40:
				encode40(in__, currentPos, code, out, tmpoutpos);
				break;
			case 41:
				encode41(in__, currentPos, code, out, tmpoutpos);
				break;
			case 42:
				encode42(in__, currentPos, code, out, tmpoutpos);
				break;
			case 43:
				encode43(in__, currentPos, code, out, tmpoutpos);
				break;
			case 44:
				encode44(in__, currentPos, code, out, tmpoutpos);
				break;
			case 45:
				encode45(in__, currentPos, code, out, tmpoutpos);
				break;
			case 46:
				encode46(in__, currentPos, code, out, tmpoutpos);
				break;
			case 47:
				encode47(in__, currentPos, code, out, tmpoutpos);
				break;
			case 48:
				encode48(in__, currentPos, code, out, tmpoutpos);
				break;
			case 49:
				encode49(in__, currentPos, code, out, tmpoutpos);
				break;
			case 50:
				encode50(in__, currentPos, code, out, tmpoutpos);
				break;
			case 51:
				encode51(in__, currentPos, code, out, tmpoutpos);
				break;
			case 52:
				encode52(in__, currentPos, code, out, tmpoutpos);
				break;
			case 53:
				encode53(in__, currentPos, code, out, tmpoutpos);
				break;
			case 54:
				encode54(in__, currentPos, code, out, tmpoutpos);
				break;
			case 55:
				encode55(in__, currentPos, code, out, tmpoutpos);
				break;
			case 56:
				encode56(in__, currentPos, code, out, tmpoutpos);
				break;
			case 57:
				encode57(in__, currentPos, code, out, tmpoutpos);
				break;
			case 58:
				encode58(in__, currentPos, code, out, tmpoutpos);
				break;
			case 59:
				encode59(in__, currentPos, code, out, tmpoutpos);
				break;
			case 60:
				encode60(in__, currentPos, code, out, tmpoutpos);
				break;
			case 61:
				encode61(in__, currentPos, code, out, tmpoutpos);
				break;
			case 62:
				encode62(in__, currentPos, code, out, tmpoutpos);
				break;
			case 63:
				encode63(in__, currentPos, code, out, tmpoutpos);
				break;
			case 64:
				encode64(in__, currentPos, code, out, tmpoutpos);
				break;
			case 65:
				encode65(in__, currentPos, code, out, tmpoutpos);
				break;
			case 66:
				encode66(in__, currentPos, code, out, tmpoutpos);
				break;
			case 67:
				encode67(in__, currentPos, code, out, tmpoutpos);
				break;
			case 68:
				encode68(in__, currentPos, code, out, tmpoutpos);
				break;
			case 69:
				encode69(in__, currentPos, code, out, tmpoutpos);
				break;
			case 70:
				encode70(in__, currentPos, code, out, tmpoutpos);
				break;
			case 71:
				encode71(in__, currentPos, code, out, tmpoutpos);
				break;
			case 72:
				encode72(in__, currentPos, code, out, tmpoutpos);
				break;
			case 73:
				encode73(in__, currentPos, code, out, tmpoutpos);
				break;
			case 74:
				encode74(in__, currentPos, code, out, tmpoutpos);
				break;
			case 75:
				encode75(in__, currentPos, code, out, tmpoutpos);
				break;
			case 76:
				encode76(in__, currentPos, code, out, tmpoutpos);
				break;
			case 77:
				encode77(in__, currentPos, code, out, tmpoutpos);
				break;
			case 78:
				encode78(in__, currentPos, code, out, tmpoutpos);
				break;
			case 79:
				encode79(in__, currentPos, code, out, tmpoutpos);
				break;
			case 80:
				encode80(in__, currentPos, code, out, tmpoutpos);
				break;
			default:
		...
		outer: while (currentPos < finalin) {
			mainloop: for (int selector = 0; selector < 8; selector++) {
				...
				int i = 0;
				for (; i < compressedNum; i++) {
					if (Util.smallerorequalthan(max, in__[currentPos + i]))
						continue mainloop;
					res = (res << b) + in__[currentPos + i];
				}
			...
			out[tmpoutpos++] = in__[currentPos++] | (selector << 28);
		}
	...
	@Override
	public void headlessUncompress(int[] in__, IntWrapper inpos, int inlength, int[] out, IntWrapper outpos, int num) {
		int currentPos = outpos.get();
		int tmpinpos = inpos.get();
		final int finalout = currentPos + num;
		while (currentPos < finalout - 2 * 28) {

			int val = in__[tmpinpos++];
			int valn = in__[tmpinpos++];
			int header = val >>> 24;
			switch (header) {
			case 0: {
