...
// 	}
	 ...
	 */
	public LongAs2IntsCodec(IntegerCODEC highPartsCodec_param, IntegerCODEC lowPartsCodec_param) {
		this.highPartsCodec = highPartsCodec_param;
		this.lowPartsCodec = lowPartsCodec_param;
	}
...
// 	}
	...
	@Override
	public void compress0(long[] in__, IntWrapper inpos, int inlength, long[] out, IntWrapper outpos) {
		if (inlength == 0) {
			return;
		}
		...
		for (int i = 0; i < inlength; i++) {
			...
			highParts[i] = RoaringIntPacking.high(in__[inPosition]);
			lowParts[i] = RoaringIntPacking.low(in__[inPosition]);
		}
	 ...
	 */
	@Override
	public void uncompress1(long[] in__, IntWrapper inpos, int inlength, long[] out, IntWrapper outpos) {
		if (inlength == 0) {
			return;
		}
		...
		int nbCompressedHighParts = RoaringIntPacking.high(in__[longIndex]);
		...
		for (int i = 0; i < nbCompressedHighParts; i++) {
			int nextInt;
			if (highPart) {
				nextInt = RoaringIntPacking.high(in__[longIndex + (i + 1) / 2]);
			} else {
				nextInt = RoaringIntPacking.low(in__[longIndex + (i + 1) / 2]);
			}
		...
		if (highPart) {
			nbCompressedLowParts = RoaringIntPacking.high(in__[intIndexNbCompressedLowParts / 2]);
		} else {
			nbCompressedLowParts = RoaringIntPacking.low(in__[intIndexNbCompressedLowParts / 2]);
		}
		highPart = !highPart;
		...
		for (int i = 0; i < nbCompressedLowParts; i++) {
			int nextInt;
			if (highPart) {
				nextInt = RoaringIntPacking.high(in__[(intIndexNbCompressedLowParts + 1 + i) / 2]);
			} else {
				nextInt = RoaringIntPacking.low(in__[(intIndexNbCompressedLowParts + 1 + i) / 2]);
			}
