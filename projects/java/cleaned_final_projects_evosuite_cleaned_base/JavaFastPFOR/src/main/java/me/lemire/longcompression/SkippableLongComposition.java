...
public class SkippableLongComposition implements SkippableLongCODEC {
    ...
    public void headlessCompress(long[] in__, IntWrapper inpos, int inlength, long[] out,
        ...
        F1.headlessCompress(in__, inpos, inlength, out, outpos);
        if (outpos.get() == outposInit) {
            out[outposInit] = 0;
            outpos.increment();
        }
        inlength -= inpos.get() - init__;
        F2.headlessCompress(in__, inpos, inlength, out, outpos);
    }
    ...
    public void headlessUncompress(long[] in__, IntWrapper inpos, int inlength, long[] out,
        ...
        int init__ = inpos.get();
        F1.headlessUncompress(in__, inpos, inlength, out, outpos, num);
        if (inpos.get() == init__) {
        	  inpos.increment();
        }
        inlength -= inpos.get() - init__;
        num -= outpos.get();
        F2.headlessUncompress(in__, inpos, inlength, out, outpos, num);
    }
