...
public class IntegratedBinaryPacking implements IntegratedIntegerCODEC,
    ...
    @Override
    public void compress0(int[] in__, IntWrapper inpos, int inlength, int[] out,
                          IntWrapper outpos) {
        ...
        outpos.increment();
        headlessCompress(in__, inpos, inlength, out, outpos, new IntWrapper(0));
    }

    @Override
    public void uncompress0(int[] in__, IntWrapper inpos, int inlength, int[] out,
                            IntWrapper outpos) {
        if (inlength == 0)
            return;
        final int outlength = in__[inpos.get()];
        inpos.increment();
        headlessUncompress(in__,inpos,inlength,out,outpos,outlength, new IntWrapper(0));
    }
    ...
    public void headlessCompress(int[] in__, IntWrapper inpos, int inlength,
        ...
        initvalue.set(in__[inpos.get()+inlength -1]);
        int s = inpos.get();
        for (; s + BLOCK_SIZE * 4 - 1 < inpos.get() + inlength; s += BLOCK_SIZE * 4) {
            final int mbits1 = Util.maxdiffbits(initoffset, in__, s, BLOCK_SIZE);
            int initoffset2 = in__[s + 31];
            final int mbits2 = Util.maxdiffbits(initoffset2, in__, s + BLOCK_SIZE, BLOCK_SIZE);
            int initoffset3 = in__[s + BLOCK_SIZE + 31];
            final int mbits3 = Util
                    .maxdiffbits(initoffset3, in__, s + 2 * BLOCK_SIZE, BLOCK_SIZE);
            int initoffset4 = in__[s + 2 * BLOCK_SIZE + 31];
            final int mbits4 = Util
                    .maxdiffbits(initoffset4, in__, s + 3 * BLOCK_SIZE, BLOCK_SIZE);
            out[tmpoutpos++] = (mbits1 << 24) | (mbits2 << 16) | (mbits3 << 8)
                    | (mbits4);
            IntegratedBitPacking.integratedpack(initoffset, in__, s, out,
                    tmpoutpos, mbits1);
            tmpoutpos += mbits1;
            IntegratedBitPacking.integratedpack(initoffset2, in__, s + BLOCK_SIZE, out,
                    tmpoutpos, mbits2);
            tmpoutpos += mbits2;
            IntegratedBitPacking.integratedpack(initoffset3, in__, s + 2 * BLOCK_SIZE,
                    out, tmpoutpos, mbits3);
            tmpoutpos += mbits3;
            IntegratedBitPacking.integratedpack(initoffset4, in__, s + 3 * BLOCK_SIZE,
                    out, tmpoutpos, mbits4);
            tmpoutpos += mbits4;
            initoffset = in__[s + 3 * BLOCK_SIZE + 31];
        }
        for (; s < inpos.get() + inlength; s += BLOCK_SIZE ) {
            final int mbits = Util.maxdiffbits(initoffset, in__, s, BLOCK_SIZE);
            out[tmpoutpos++] = mbits;
            IntegratedBitPacking.integratedpack(initoffset, in__, s, out,
                    tmpoutpos, mbits);
            tmpoutpos += mbits;
            initoffset = in__[s + 31];
        }
        inpos.add(inlength);
    ...
    public void headlessUncompress(int[] in__, IntWrapper inpos, int inlength,
        ...
        for (; s + BLOCK_SIZE * 4 - 1 < outpos.get() + outlength; s += BLOCK_SIZE * 4) {
            final int mbits1 = (in__[tmpinpos] >>> 24);
            final int mbits2 = (in__[tmpinpos] >>> 16) & 0xFF;
            final int mbits3 = (in__[tmpinpos] >>> 8) & 0xFF;
            final int mbits4 = (in__[tmpinpos]) & 0xFF;

            ++tmpinpos;
            IntegratedBitPacking.integratedunpack(initoffset, in__, tmpinpos,
                    out, s, mbits1);
            tmpinpos += mbits1;
            initoffset = out[s + 31];
            IntegratedBitPacking.integratedunpack(initoffset, in__, tmpinpos,
