...
//         }
        ...
        private void getBestBFromData(int[] in__, int pos) {
                Arrays.fill(freqs, 0);
                for (int k = pos, k_end = pos + BLOCK_SIZE; k < k_end; ++k) {
                        freqs[Util.bits(in__[k])]++;
                }
        ...
        private void encodePage(int[] in__, IntWrapper inpos, int thissize,
                int[] out, IntWrapper outpos) {
                ...
                for (final int finalinpos = tmpinpos + thissize - BLOCK_SIZE; tmpinpos <= finalinpos; tmpinpos += BLOCK_SIZE) {
                    getBestBFromData(in__, tmpinpos);
                        ...
                        if (bestbbestcexceptmaxb[1] > 0) {
                                ...
                                for (int k = 0; k < BLOCK_SIZE; ++k) {
                                        if ((in__[k + tmpinpos] >>> bestbbestcexceptmaxb[0]) != 0) {
                                                // we have an exception
                                                byteContainer.put((byte) k);
                                                dataTobePacked[index][dataPointers[index]++] = in__[k
                                                        + tmpinpos] >>> tmpbestb;
                                        }
                                }
                        ...
                        for (int k = 0; k < BLOCK_SIZE; k += 32) {
                                BitPacking.fastpack(in__, tmpinpos + k, out,
                                        tmpoutpos, tmpbestb);
                                tmpoutpos += tmpbestb;
                        }
                }
        ...
        private void decodePage(int[] in__, IntWrapper inpos, int[] out,
                ...
                byteContainer.clear();
                byteContainer.asIntBuffer().put(in__, inexcept, (bytesize + 3) / 4);
                ...
                for (int k = 2; k <= 32; ++k) {
                        if ((bitmap & (1 << (k - 1))) != 0) {
                                ...
                                if(inexcept + roundedup/32*k <= in__.length) {
                                    int j = 0;
                                    for (; j < size; j += 32) {
                                        BitPacking.fastunpack(in__, inexcept,
                ...
                for (int run = 0, run_end = thissize / BLOCK_SIZE; run < run_end; ++run, tmpoutpos += BLOCK_SIZE) {
                        ...
                        for (int k = 0; k < BLOCK_SIZE; k += 32) {
                                BitPacking.fastunpack(in__, tmpinpos, out,
                                        tmpoutpos + k, b);
                                tmpinpos += b;
                        }
                        if (cexcept > 0) {
