...
public final class BinaryPacking implements IntegerCODEC, SkippableIntegerCODEC {
        ...
        @Override
        public void compress0(int[] in__, IntWrapper inpos, int inlength,
        ...
        public void headlessCompress(int[] in__, IntWrapper inpos, int inlength,
            ...
            for (; s + BLOCK_SIZE * 4 - 1 < inpos.get() + inlength; s += BLOCK_SIZE * 4) {
                final int mbits1 = Util.maxbits(in__, s, BLOCK_SIZE);
                final int mbits2 = Util.maxbits(in__, s + BLOCK_SIZE, BLOCK_SIZE);
                final int mbits3 = Util.maxbits(in__, s + 2 * BLOCK_SIZE, BLOCK_SIZE);
                final int mbits4 = Util.maxbits(in__, s + 3 * BLOCK_SIZE, BLOCK_SIZE);
                out[tmpoutpos++] = (mbits1 << 24) | (mbits2 << 16)
                        | (mbits3 << 8) | (mbits4);
                BitPacking.fastpackwithoutmask(in__, s, out, tmpoutpos,
                        mbits1);
                tmpoutpos += mbits1;
                BitPacking.fastpackwithoutmask(in__, s + BLOCK_SIZE, out,
                        tmpoutpos, mbits2);
                tmpoutpos += mbits2;
                BitPacking.fastpackwithoutmask(in__, s + 2 * BLOCK_SIZE, out,
                        tmpoutpos, mbits3);
                tmpoutpos += mbits3;
                BitPacking.fastpackwithoutmask(in__, s + 3 * BLOCK_SIZE, out,
                        tmpoutpos, mbits4);
                tmpoutpos += mbits4;
            }
            for (; s < inpos.get() + inlength; s += BLOCK_SIZE ) {
                final int mbits = Util.maxbits(in__, s, BLOCK_SIZE);
                out[tmpoutpos++] = mbits;
                BitPacking.fastpackwithoutmask(in__, s, out, tmpoutpos,
                        mbits);
                tmpoutpos += mbits;
        ...
        @Override
        public void uncompress0(int[] in__, IntWrapper inpos, int inlength,
                                int[] out, IntWrapper outpos) {
                if (inlength == 0)
                        return;
        ...
        public void headlessUncompress(int[] in__, IntWrapper inpos, int inlength,
            ...
            for (; s + BLOCK_SIZE * 4 - 1 < outpos.get() + outlength; s += BLOCK_SIZE * 4) {
                ...
                ++tmpinpos;
                BitPacking.fastunpack(in__, tmpinpos, out, s, mbits1);
                tmpinpos += mbits1;
                BitPacking
                .fastunpack(in__, tmpinpos, out, s + BLOCK_SIZE, mbits2);
                tmpinpos += mbits2;
                BitPacking.fastunpack(in__, tmpinpos, out, s + 2 * BLOCK_SIZE,
                        mbits3);
                tmpinpos += mbits3;
                BitPacking.fastunpack(in__, tmpinpos, out, s + 3 * BLOCK_SIZE,
                        mbits4);
                tmpinpos += mbits4;
            }
            for (; s < outpos.get() + outlength; s += BLOCK_SIZE ) {
                ...
                ++tmpinpos;
                BitPacking.fastunpack(in__, tmpinpos, out, s, mbits);
                tmpinpos += mbits;
            }
