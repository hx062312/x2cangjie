...
public class VectorFastPFOR implements IntegerCODEC, SkippableIntegerCODEC {
  ...
  public void headlessCompress(int[] in__, IntWrapper inpos, int inlength,
    ...
    while (inpos.get() != finalinpos) {
      ...
      encodePage(in__, inpos, thissize, out, outpos);
    }
  }

  private void getBestBitSize(int[] in__, int pos, int index) {
    Arrays.fill(freqs, 0);
    for (int i = pos, limit = pos + BLOCK_SIZE; i < limit; i++) {
      freqs[32 - Integer.numberOfLeadingZeros(in__[i])]++;
    }
    bem[index] = 32;
  ...
  private void encodePage(int[] in__, IntWrapper inpos, int thissize, int[] out,
                          IntWrapper outpos) {
    ...
    for (; tmpinpos <= finalinpos; tmpinpos += BLOCK_SIZE) {
      getBestBitSize(in__, tmpinpos, bindex);
      ...
      if (tmpexcept > 0) {
        ...
        for (int k = 0; k < BLOCK_SIZE; ++k) {
          if ((in__[k + tmpinpos] >>> tmpbestb) != 0) {
            // we have an exception
            bem[bindex++] = (byte)k;
            dataTobePacked[index][dataPointers[index]++] =
                in__[k + tmpinpos] >>> tmpbestb;
          }
        }
      } else {
        bindex += 2;
      }
      VectorBitPacker.fastpack(in__, tmpinpos, out, tmpoutpos, tmpbestb);
  ...
  public void headlessUncompress(int[] in__, IntWrapper inpos, int inlength,
    ...
    while (outpos.get() != finalout) {
      ...
      decodePage(in__, inpos, out, outpos, thissize);
    }
  }

  private void loadMetaData(int[] in__, int inexcept, int bytesize) {
    // Arrays.fill(bem, (byte)0);
    int len = (bytesize + 3) / 4;
    int lc = 0;
    for (int i = 0; i < len; i++) {
      bem[lc++] = (byte)(in__[inexcept + i]);
      bem[lc++] = (byte)(in__[inexcept + i] >>> 8);
      bem[lc++] = (byte)(in__[inexcept + i] >>> 16);
      bem[lc++] = (byte)(in__[inexcept + i] >>> 24);
    }
  }

  private void decodePage(int[] in__, IntWrapper inpos, int[] out,
                          IntWrapper outpos, int thissize) {
    final int initpos = inpos.get();
    final int wheremeta = in__[inpos.get()];
    inpos.increment();
    int inexcept = initpos + wheremeta;

    final int bytesize = in__[inexcept++];
    loadMetaData(in__, inexcept, bytesize);
    inexcept += (bytesize + 3) / 4;
    final int bitmap = in__[inexcept++];
    for (int k = 2; k <= 32; ++k) {
      if ((bitmap & (1 << (k - 1))) != 0) {
        int size = in__[inexcept++];
        ...
        if (inexcept + roundedup / 32 * k <= in__.length) {
          int j = 0;
          int len = (size / BLOCK_SIZE) * BLOCK_SIZE;
          for (; j < len; j += BLOCK_SIZE) {
            VectorBitPacker.fastunpack(in__, inexcept, dataTobePacked[k], j, k);
            inexcept += INTS_PER_BLOCK * k;
          }
          int r = size % BLOCK_SIZE;
          inexcept = VectorBitPacker.slowunpack(in__, inexcept, dataTobePacked[k],
                                                j, r, k);
        } else {
          int j = 0;
          ...
          System.arraycopy(in__, inexcept, buf, 0, in__.length - inexcept);
    ...
    for (int run = 0, run_end = thissize / BLOCK_SIZE; run < run_end;
      ...
      VectorBitPacker.fastunpack(in__, tmpinpos, out, tmpoutpos, b);
