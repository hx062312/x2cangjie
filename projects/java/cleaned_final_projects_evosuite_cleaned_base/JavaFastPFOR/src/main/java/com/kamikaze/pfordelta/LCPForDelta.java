...
public class LCPForDelta {
         ...
         */
        public static final int readBits(int[] in__, final int inOffset,
                final int bits) {
                final int index = inOffset >>> 5;
                final int skip = inOffset & 0x1f;
                int val = in__[index] >>> skip;
                if (32 - skip < bits) {
                        val |= (in__[index + 1] << (32 - skip));
                }
                return val & (0xffffffff >>> (32 - bits));
        }

        protected static final int readBitsWithBuffer(int[] in__,
