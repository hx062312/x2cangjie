...
public abstract class BaseNCodec implements BinaryEncoder, BinaryDecoder {
     ...
     * @since 1.7
     */
    static class BaseNCodec_Context {
     ...
     * @throws OutOfMemoryError if the {@code minCapacity} is negative
     */
    private static byte[] resizeBuffer(final BaseNCodec_Context context, final int minCapacity) {
    ...
    protected BaseNCodec(
            int constructorId,
            final int unencodedBlockSize_param,
            final int encodedBlockSize_param,
            final int lineLength_param,
            final int chunkSeparatorLength_param,
            final byte pad_param,
            final CodecPolicy decodingPolicy_param) {
        this.unencodedBlockSize = unencodedBlockSize_param;
        this.encodedBlockSize = encodedBlockSize_param;
        final boolean useChunking = lineLength_param > 0 && chunkSeparatorLength_param > 0;
        this.lineLength = useChunking ? (lineLength_param / encodedBlockSize_param) * encodedBlockSize_param : 0;
        this.chunkSeparatorLength = chunkSeparatorLength_param;
        if (constructorId == 0) {
        ...
        if (constructorId == 2) {
            this.decodingPolicy = Objects.requireNonNull(decodingPolicy_param, "codecPolicy");
        } else {
     ...
     */
    int available(final BaseNCodec_Context context) { // package protected for access from I/O streams
    ...
    public byte[] decode0(final byte[] pArray) {
        ...
        final BaseNCodec_Context context = new BaseNCodec_Context();
    ...
    abstract void decode1(byte[] pArray, int i, int length, BaseNCodec_Context context);
    ...
    abstract void encode2(byte[] pArray, int i, int length, BaseNCodec_Context context);
     ...
     * @return the buffer
     */
    protected byte[] ensureBufferSize(final int size, final BaseNCodec_Context context) {
     ...
     */
    boolean hasData(final BaseNCodec_Context context) { // package protected for access from I/O streams
     ...
     */
    int readResults(final byte[] b, final int bPos, final int bAvail, final BaseNCodec_Context context) {
        if (hasData(context)) {
