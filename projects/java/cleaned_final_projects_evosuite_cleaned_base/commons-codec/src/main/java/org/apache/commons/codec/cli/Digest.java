...
public class Digest {
    ...
    private Digest(final String[] args_param) {
        if (args_param == null) {
            throw new IllegalArgumentException("args");
        }
        final int argsLength = args_param.length;
        if (argsLength == 0) {
        ...
        algorithm = args_param[0];
        if (argsLength <= 1) {
            inputs = null;
        } else {
            inputs = new String[argsLength - 1];
            System.arraycopy(args_param, 1, inputs, 0, inputs.length);
        }
    }
