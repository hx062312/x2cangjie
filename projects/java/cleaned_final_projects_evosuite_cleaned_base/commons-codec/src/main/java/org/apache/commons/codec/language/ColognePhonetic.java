...
public class ColognePhonetic implements StringEncoder {
     ...
     */
    abstract class ColognePhonetic_CologneBuffer {

        protected final char[] data;

        protected int length = 0;

        public ColognePhonetic_CologneBuffer(int constructorId, final char[] data_param, final int buffSize) {
            if (constructorId == 0) {
                this.data = new char[buffSize];
                this.length = 0;
            } else {
                this.data = data_param;
                this.length = data_param.length;
            }
        }

        protected abstract char[] copyData(int start, final int length_param);
    ...
    private class ColognePhonetic_CologneOutputBuffer extends ColognePhonetic_CologneBuffer {

        private char lastCode;

        public ColognePhonetic_CologneOutputBuffer(final int buffSize) {
        ...
        @Override
        protected char[] copyData(final int start, final int length_param) {
            final char[] newData = new char[length_param];
            System.arraycopy(data, start, newData, 0, length_param);
            return newData;
        }
    }

    private class ColognePhonetic_CologneInputBuffer extends ColognePhonetic_CologneBuffer {

        public ColognePhonetic_CologneInputBuffer(final char[] data_param) {
            super(1, data_param, 0);
        }

        @Override
        protected char[] copyData(final int start, final int length_param) {
            final char[] newData = new char[length_param];
            System.arraycopy(data, data.length - this.length + start, newData, 0, length_param);
            return newData;
        }
    ...
    public String colognePhonetic(final String text) {
        ...
        final ColognePhonetic_CologneInputBuffer input = new ColognePhonetic_CologneInputBuffer(preprocess(text));
        final ColognePhonetic_CologneOutputBuffer output = new ColognePhonetic_CologneOutputBuffer(input.length() * 2);
