...
public final class Blake3 {
    ...
    private final Blake3_EngineState engineState;

    private Blake3(final int[] key, final int flags) {
        engineState = new Blake3_EngineState(key, flags);
    }
     ...
     */
    public void update0(final byte[] in__) {
        Objects.requireNonNull(in__);
        update1(in__, 0, in__.length);
    }
     ...
     */
    public void update1(final byte[] in__, final int offset, final int length) {
        checkBufferArgs(in__, offset, length);
        engineState.inputData(in__, offset, length);
    }
    ...
    public static Blake3 initKeyDerivationFunction(final byte[] kdfContext) {
        ...
        final Blake3_EngineState kdf = new Blake3_EngineState(IV, DERIVE_KEY_CONTEXT);
    ...
    private static Blake3_Output parentOutput(
        ...
        return new Blake3_Output(key.clone(), blockWords, 0, BLOCK_LEN, flags | PARENT);
    }
     ...
     */
    private static class Blake3_Output {
        ...
        Blake3_Output(
                final int[] inputChainingValue_param,
                final int[] blockWords_param,
                final long counter_param,
                final int blockLength_param,
                final int flags_param) {
            this.inputChainingValue = inputChainingValue_param;
            this.blockWords = blockWords_param;
            this.counter = counter_param;
            this.blockLength = blockLength_param;
            this.flags = flags_param;
        }
    ...
    private static class Blake3_ChunkState {
        ...
        Blake3_ChunkState(final int[] key, final long chunkCounter_param, final int flags_param) {
            chainingValue = key;
            this.chunkCounter = chunkCounter_param;
            this.flags = flags_param;
        }
        ...
        Blake3_Output output() {
            ...
            return new Blake3_Output(chainingValue, blockWords, chunkCounter, blockLength, outputFlags);
        }
    }

    private static class Blake3_EngineState {
        private final int[] key;
        private final int flags;
        private final int[][] cvStack = new int[54][];
        private int stackLen;
        private Blake3_ChunkState state;

        Blake3_EngineState(final int[] key_param, final int flags_param) {
            this.key = key_param;
            this.flags = flags_param;
            state = new Blake3_ChunkState(key_param, 0, flags_param);
        }

        void inputData(final byte[] in__, int offset, int length) {
            while (length > 0) {
                if (state.length() == CHUNK_LEN) {
                    ...
                    state = new Blake3_ChunkState(key, totalChunks, flags);
                }

                final int want = CHUNK_LEN - state.length();
                final int take = Math.min(want, length);
                state.update(in__, offset, take);
                offset += take;
                length -= take;
            }
        }
        ...
        void outputHash(final byte[] out, final int offset, final int length) {
            Blake3_Output output = state.output();
        ...
        void reset() {
            ...
            state = new Blake3_ChunkState(key, 0, flags);
        }
