...
final class FlowNetworkHandler<V, E, W>
    ...
    FlowNetworkHandler(
            DirectedGraph<V, E> flowNetwork_param,
            V source_param,
            V target_param,
            OrderedMonoid<W> weightOperations_param,
            Mapper<E, W> weightedEdges_param) {
        this.flowNetwork = flowNetwork_param;
        ...
        maxFlow = weightOperations_param.identity();

        for (E edge : flowNetwork_param.getEdges()) {
            residualEdgeCapacities.put(edge, weightedEdges_param.map(edge));
        }
