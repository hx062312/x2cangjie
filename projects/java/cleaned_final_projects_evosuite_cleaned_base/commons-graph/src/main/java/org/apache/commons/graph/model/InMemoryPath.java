...
public class InMemoryPath<V, E> implements Path<V, E> {
     ...
     */
    public InMemoryPath(V start, V target_param) {
        this.source = checkNotNull(start, "Path source cannot be null");
        this.target = checkNotNull(target_param, "Path target cannot be null");
    }
    ...
    /** {@inheritDoc} */
    public E getEdge(V source_param, V target_param) {
        return indexedEdges.get(new VertexPair<V>(source_param, target_param));
    }
