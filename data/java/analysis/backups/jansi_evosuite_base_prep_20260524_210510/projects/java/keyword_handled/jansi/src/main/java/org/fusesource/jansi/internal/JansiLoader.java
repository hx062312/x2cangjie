...
public class JansiLoader {
    ...
    private static int readNBytes(InputStream in__, byte[] b) throws IOException {
        int n = 0;
        int len = b.length;
        while (n < len) {
            int count = in__.read(b, n, len - n);
            if (count <= 0) break;
            n += count;
        }
        return n;
    ...
    private static boolean extractAndLoadLibraryFile(
        ...
        try {
            ...
            try (InputStream in__ = JansiLoader.class.getResourceAsStream(nativeLibraryFilePath)) {
                if (!extractedLckFile.exists()) {
                    new FileOutputStream(extractedLckFile).close();
                }
                Files.copy(in__, extractedLibFile.toPath(), StandardCopyOption.REPLACE_EXISTING);
            } finally {
