...
from src.java.skeleton.get_dependencies import get_dependencies
...
def _load_third_party_libraries(
    path="data/java/type_resolution/java_base_third_party_libraries.json",
):
    ...
    for lib_name, lib_info in third_party_libraries.items():
        ...
        for lib_prefix in _third_party_import_prefixes(lib_info):
            for import_prefix in import_prefixes:
                if import_prefix == lib_prefix or import_prefix.startswith(
                    f"{lib_prefix}."
                ):
                    used.add(lib_name)
                    break
...
def _render_dependency_lines(third_party_libraries, used_third_party_libs):
    ...
    for lib_name in sorted(used_third_party_libs):
        ...
        dependency = (
            lib_info.get("dependency", {}) if isinstance(lib_info, dict) else {}
        )
...
def _generate_cjpm_content(
    cjpm_name,
    output_type,
    include_stdx,
    third_party_libraries=None,
    used_third_party_libs=None,
):
    link_option = _stdx_link_option() if include_stdx else ""
    dependency_lines = _render_dependency_lines(
        third_party_libraries or {}, used_third_party_libs or set()
    )
...
def get_method_type_params(method_info):
    ...
    for rt in method_info.get("return_types", []):
        if (
            isinstance(rt, str)
            and rt.strip().startswith("<")
            and rt.strip().endswith(">")
        ):
...
def generate_field_skeleton(field_info, field_key, type_map):
    ...
    skeleton = (
        f"    {field_prefix}{field_name}: {field_type} = throw Exception('TODO')\n"
    )
...
def generate_class_skeleton(
    ...
    for field_key in sorted(class_info.get("fields", {})):
        ...
        field_skeleton, field_partial = generate_field_skeleton(
            field_info, field_key, type_map
        )
    ...
    if class_info.get("static_initializers"):
        ...
        for static_init_key, static_init_info in class_info.get(
            "static_initializers", {}
        ).items():
            static_init_skeleton, static_init_partial = (
                generate_static_initializer_skeleton(static_init_info, static_init_key)
            )
...
def generate_interface_skeleton(
    ...
    extends = class_info.get("extends", [])
    parent_names = _get_interface_parents(
        class_name, extends, class_to_package, type_map
    )
    if parent_names:
        declaration = (
            f"{interface_mod}interface {class_name} <: {' & '.join(parent_names)} {{\n"
        )
    else:
...
def _is_test_class(class_info):
    ...
    return any(
        "@Test"
        in [x.split("(")[0] for x in class_info["methods"][m].get("annotations", [])]
...
def _check_needs_super_call(extends, all_schema_classes):
    ...
    parent_constructors = [
        pm
        for pm_key, pm in all_schema_classes[parent_short].get("methods", {}).items()
...
def _iter_java_type_names(java_type_expr):
    ...
    if "<" in java_type_expr and java_type_expr.endswith(">"):
        ...
        if base_type:
            yield base_type
        generic_part = java_type_expr[
            java_type_expr.index("<") + 1 : java_type_expr.rindex(">")
        ]
...
def generate_imports_skeleton(
    ...
    _add_lib_imports(
        cangjie_imports, schema, class_order, type_map, java_type_imports, cjpm_name
    )
    ...
    for imp in cangjie_imports:
        if imp.startswith("import ") and not imp.startswith(
            ("import std.", "import ohos.")
        ):
...
def _add_project_imports(
    ...
    for class_key in class_order:
        ...
        for ref_type in class_info.get("extends", []) + class_info.get(
            "implements", []
        ):
...
def _add_lib_imports(
    cangjie_imports, schema, class_order, type_map, java_type_imports, cjpm_name
):
    """Add imports for library types (std + type_translations)."""
    uses_any_hashable = False
    import_map = (
        schema.get("import_map", {})
        if isinstance(schema.get("import_map", {}), dict)
        else {}
    )
    ...
    for class_key in class_order:
        ...
        for field_key, field_info in class_info.get("fields", {}).items():
            for t in field_info.get("types", []):
                ...
                _add_java_type_imports(
                    t, cangjie_imports, java_type_imports, import_map
                )
        for method_key, method_info in class_info.get("methods", {}).items():
            for rt in method_info.get("return_types", []):
                ...
                uses_any_hashable = uses_any_hashable or "AnyHashable" in cangjie_type
                _add_java_type_imports(
                    rt, cangjie_imports, java_type_imports, import_map
                )
            for p in method_info.get("parameters", []):
                ...
                uses_any_hashable = uses_any_hashable or "AnyHashable" in cangjie_type
                _add_java_type_imports(
                    source_type, cangjie_imports, java_type_imports, import_map
                )
        ...
        for fragment_type in ["fields", "methods"]:
            for frag_key, frag_data in class_info.get(fragment_type, {}).items():
                for tv in ["types", "return_types", "parameters", "body_types"]:
                    for tid, tdata in (
                        frag_data.get("type_translations", {}).get(tv, {}).items()
                    ):
                        ...
                        uses_any_hashable = uses_any_hashable or "AnyHashable" in str(
                            value
                        )
...
def _add_java_type_imports(
    java_type_expr, cangjie_imports, java_type_imports, import_map
):
...
def generate_one_file_skeleton(
    schema,
    schema_fname,
    schema_path,
    cjpm_name,
    ...
    for class_key in class_order:
        ...
        schema["classes"][class_key]["cangjie_class_declaration"] = class_info.get(
    ...
    if (
        original_sub_path
        and collapsed_subpaths
        and original_sub_path in collapsed_subpaths
    ):
...
def main(args):
    ...
    schema_filter = lambda schema_fname: (
        include_tests or not _is_test_schema_name(schema_fname)
    )
    custom_types = get_custom_types(schema_dir, schema_filter=schema_filter)
    type_map.update(
        get_custom_type_translation_map(schema_dir, schema_filter=schema_filter)
    )
    ...
    for schema_fname in os.listdir(schema_dir):
        if not schema_fname.endswith(".json"):
            continue
        if (
            f"{args.project}.src.main" not in schema_fname
            and f"{args.project}.src.test" not in schema_fname
        ):
            continue
    ...
    changed_subpaths = {
        raw: effective
        for raw, effective in effective_subpaths.items()
    ...
    for schema_fname, schema_path, schema in all_schemas:
        cangjie_pkg = _get_cangjie_package(
            schema.get("path", ""), cjpm_name, effective_subpaths
        )
    ...
    for schema_fname, schema_path, schema in all_schemas:
        ...
        file_uses_any_hashable, file_uses_stdx, file_third_party_libs = (
    ...
    cjpm_content = _generate_cjpm_content(
        cjpm_name, output_type, uses_stdx, third_party_libraries, used_third_party_libs
    )
    ...
    if uses_any_hashable:
        ...
        runtime_support.inject_any_hashable(
            Path(translations_skeleton_dir) / "src", cjpm_name
        )

    if render_shim_file(
        args.project, cjpm_name, [skeletons_dir, translations_skeleton_dir]
    ):
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Create Cangjie skeleton from Java schema"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--suffix", type=str, dest="suffix", help="suffix (e.g., _decomposed_tests)"
    )
    parser.add_argument(
