...
from src.java.skeleton.create_schema import get_schema_file
...
def get_dependencies(args):
    ...
    call_graph = [
        line.strip()[2:] for line in call_graph if line.strip().startswith("C:")
    ]

    dependencies = {}
    for schema_file in os.listdir(args.schemas_dir):
        partial_path = schema_file.replace(f"{args.project}.", "", 1).replace(
            ".json", ""
        )
