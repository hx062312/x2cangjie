...
# ---------------------------------------------------------------------------
# schema template — centralized field definitions, edit configs/schema_template.json
# ---------------------------------------------------------------------------
_template = None


def load_schema_template():
    """Load schema field templates from config file. Cached after first call."""
    global _template
                ...
                for subchild in child.children:
                    if subchild.type == "type_list":
                        for subsubchild in subchild.children:
                            ...
                            extend_classes.append(
                                extract_text_by_bytes(
                                    code, subsubchild.start_byte, subsubchild.end_byte
                                )
                            )
    ...
    def get_method_body_types(self, node, code):
        ...
        def recurse(node):
            if (
                ...
                or "type_identifier" in node.type
                and node.parent.type
                not in ["generic_type", "array_type", "type_arguments"]
            ):
                type_content = extract_text_by_bytes(
                    ...
                    if subchild.type == "formal_parameter":
                        ...
                        for subsubchild in subchild.children:
                            if subsubchild.type == "modifiers":
                                parameter_details["modifier"] = extract_text_by_bytes(
                                    code, subsubchild.start_byte, subsubchild.end_byte
                                )
                            if "type" in subsubchild.type:
                                parameter_details["type"] = extract_text_by_bytes(
                                    code, subsubchild.start_byte, subsubchild.end_byte
                                )
                            if subsubchild.type == "identifier":
                                parameter_details["name"] = extract_text_by_bytes(
                                    code, subsubchild.start_byte, subsubchild.end_byte
                                )
...
def get_language_parser(language):
    ...
    try:
        ...
        # tree-sitter 0.20.x: Language(library_path, name)
        lib_path = os.path.join(
            os.path.dirname(tree_sitter_java.__file__), "_binding.abi3.so"
        )
        LANGUAGE = Language(lib_path, "java")
    except Exception:
...
def process_method_declaration(subchild, code, schema, class_key):
    ...
    signature = (
        ...
        + "("
        + ", ".join(
            [f"{x['modifier']} {x['type']} {x['name']}" for x in method_parameters]
        )
        + ")"
    )
    ...
    tpl = load_schema_template()
    entry = copy.deepcopy(tpl["method"])
    entry.update(
        {
            "start": method_start_line,
            "end": method_end_line,
            "body": extract_text_by_bytes(
                code, subchild.start_byte, subchild.end_byte
            ).split("\n"),
            "is_constructor": True
            if subchild.type == "constructor_declaration"
            else False,
            "annotations": [x for x in method_modifiers if x.startswith("@")],
            "modifiers": [
                x
                for x in method_modifiers
    ...
    schema["classes"][class_key]["methods"][method_key] = entry

    default_type_entry = tpl["type_translation_entry"]

    for type_ in method_return_types:
        schema["classes"][class_key]["methods"][method_key]["type_translations"][
            "return_types"
        ].setdefault(type_, copy.deepcopy(default_type_entry))
    ...
    for parameter in method_parameters:
        schema["classes"][class_key]["methods"][method_key]["type_translations"][
            ...
            f"{parameter['modifier']}|{parameter['type']}|{parameter['name']}",
            copy.deepcopy(default_type_entry),
        )
...
def process_enum_constant(subchild, code, schema, class_key):
    ...
    for subsubchild in subchild.children:
        if subsubchild.type == "identifier":
            enum_constant_name = extract_text_by_bytes(
                code, subsubchild.start_byte, subsubchild.end_byte
            )
        if subsubchild.type == "argument_list":
            enum_constant_args = extract_text_by_bytes(
                code, subsubchild.start_byte, subsubchild.end_byte
            )
    ...
    enum_constant_key = (
        f"{enum_constant_start_line}-{enum_constant_end_line}:{enum_constant_name}"
    )

    schema["classes"][class_key]["fields"].setdefault(enum_constant_key, {})
    tpl = load_schema_template()
    entry = copy.deepcopy(tpl["enum_constant"])
    entry.update(
        {
    ...
    schema["classes"][class_key]["fields"][enum_constant_key] = entry
...
def process_field_declaration(subchild, code, schema, class_key):
    ...
    schema["classes"][class_key]["fields"][field_key] = entry

    default_type_entry = tpl["type_translation_entry"]
    for type_ in field_types:
        schema["classes"][class_key]["fields"][field_key]["type_translations"][
            "types"
        ].setdefault(type_, copy.deepcopy(default_type_entry))
...
def process_static_initializer(subchild, code, schema, class_key):
    ...
    static_initializer_key = (
        f"{static_initializer_start_line}-{static_initializer_end_line}:run_static_init"
    )

    tpl = load_schema_template()
    entry = copy.deepcopy(tpl["static_initializer"])
    entry.update(
        {
    ...
    schema["classes"][class_key]["static_initializers"][static_initializer_key] = entry
...
def decompose(node, code, file):
    ...
    def decompose_node(node):
        if node.type == "import_declaration":
            ...
            import_entry = copy.deepcopy(tpl["import"])
            import_entry.update(
                {
            ...
            schema["imports"][import_key] = import_entry
            ...
            class_entry = copy.deepcopy(tpl["class"])
            class_entry.update(
                {
            ...
            schema["classes"][class_key] = class_entry
                    ...
                    for subchild in child.children:
                        if subchild.type == "enum_constant":
                            ...
                            process_method_declaration(
                                subchild, code, schema, class_key
                            )
                            ...
                            for subsubchild in subchild.children:
                                if subsubchild.type == "field_declaration":
                                    process_field_declaration(
                                        subsubchild, code, schema, class_key
                                    )

                                elif subsubchild.type in [
                                    "method_declaration",
                                    "constructor_declaration",
                                ]:
                                    process_method_declaration(
...
# ---------------------------------------------------------------------------
# Post-process all generated schemas: annotate overload / override / needs_open
# ---------------------------------------------------------------------------
def _annotate_schemas(args):
    """Load all schemas, detect method overloads & overrides, write back."""
    all_schema_classes = {}  # class_name -> {"extends": [...], "methods": {...}}
    schema_files = {}  # class_name -> schema file path

    for fname in os.listdir(args.schemas_dir):
        if not fname.endswith(".json"):
            continue
        spath = os.path.join(args.schemas_dir, fname)
        with open(spath, "r") as f:
            s = json.load(f)
        for ckey, cinfo in s.get("classes", {}).items():
            cname = ckey.split(":")[-1]
            all_schema_classes[cname] = {
                "extends": cinfo.get("extends", []),
                "methods": cinfo.get("methods", {}),
            }
            schema_files[cname] = spath
    ...
    for fname in os.listdir(args.schemas_dir):
        ...
        for ckey, cinfo in s.get("classes", {}).items():
            ...
            # -- overload: same class, same method name, different params --
            name_counts = {}
            for mkey in cinfo.get("methods", {}):
                mname = mkey.split(":")[-1].strip()
                name_counts.setdefault(mname, []).append(mkey)
            for mname, mkeys in name_counts.items():
                if len(mkeys) > 1:
                    for mk in mkeys:
                        cinfo["methods"][mk]["is_overload"] = True
                        modified = True

            # -- override: method exists in parent class with matching signature --
            extends = cinfo.get("extends", [])
            parent_short = extends[0].split(".")[-1] if extends else None
            if parent_short and parent_short in all_schema_classes:
                for mkey, minfo in cinfo.get("methods", {}).items():
                    mname = mkey.split(":")[-1].strip()
                    child_types = [p["type"] for p in minfo.get("parameters", [])]
                    for pmkey, pminfo in (
                        all_schema_classes[parent_short].get("methods", {}).items()
                    ):
                        pmname = pmkey.split(":")[-1].strip()
                        if pmname == mname:
                            parent_types = [
                                p["type"] for p in pminfo.get("parameters", [])
                            ]
                            if child_types == parent_types:
                                minfo["is_override"] = True
                                modified = True
                                # mark parent method as needs_open
                                pspath = schema_files.get(parent_short)
                                if pspath:
                                    with open(pspath, "r") as pf:
                                        ps = json.load(pf)
                                    for pckey in ps.get("classes", {}):
                                        if pckey.split(":")[-1] == parent_short:
                                            if pmkey in ps["classes"][pckey].get(
                                                "methods", {}
                                            ):
                                                ps["classes"][pckey]["methods"][pmkey][
                                                    "needs_open"
                                                ] = True
                                                with open(pspath, "w") as pf:
...
def _populate_java_types(args):
    """Derive java_types from the calls field (already populated from callgraph)."""
    for fname in os.listdir(args.schemas_dir):
        if not fname.endswith(".json"):
            continue
        spath = os.path.join(args.schemas_dir, fname)
            ...
            for mkey, minfo in cinfo.get("methods", {}).items():
                ...
                for call in minfo.get("calls", []):
                    if len(call) < 2:
                        continue
                    if call[0] == "library":
                        # ["library", "java.lang.Character", "isJavaIdentifierPart(char)"]
                        fqcn = call[1]
                        type_name = fqcn.split(".")[-1]
                        type_path = fqcn
                        method_sig = call[2] if len(call) > 2 else ""
                        method_name = method_sig.split("(")[0]
                    else:
                        # ["schema_fname", "class_key", "method_key"]
                        schema_fname = call[0]  # e.g. "commons-cli.src.main.org.apache.commons.cli.Util"
                        class_key = call[1]      # e.g. "53-65:Util"
                        type_name = class_key.split(":")[-1]
                        type_path = schema_fname
                        method_key = call[2] if len(call) > 2 else ""
                    ...
                    if type_name and method_name:
                        ...
                        for e in entries:
                            if e["type"] == type_name:
                                ...
                                break
                        if found is None:
                            found = {"type": type_name, "path": type_path, "methods": []}
                            entries.append(found)
                        if method_name not in found["methods"]:
                            found["methods"].append(method_name)

                if entries:
                    minfo["java_types"] = entries
                    modified = True
    ...
    print(f"  [java_types] derived from calls")
...
def create_schema(args):
    ...
    java_files = get_dir_files(
        f"projects/java/cleaned_final_projects{args.suffix}/{args.project_name}"
    )
    ...
    for file in java_files:
        ...
        class_ranges = {
            ...
            for start_line, end_line in [(class_details["start"], class_details["end"])]
        }
        for class_name, class_details in schema["classes"].items():
            for other_class_name, other in schema["classes"].items():
                if class_name != other_class_name:
                    if (
                        class_ranges[class_name][0] > class_ranges[other_class_name][0]
                        and class_ranges[class_name][1]
                        < class_ranges[other_class_name][1]
                    ):
                        schema["classes"][class_name]["nested_inside"] = (
                            other_class_name
                        )
                        schema["classes"][other_class_name]["nests"].append(class_name)

        schema_name = (
            file[file.find(args.project_name) :].replace("/", ".").replace(".java", "")
        )
    ...
    for call in call_graph:
        ...
        caller_schema_name, caller_signature = (
            caller.split(":")[0],
            caller.split(":")[1],
        )
        callee_schema_name, callee_signature = (
            callee.split(":")[0],
            callee.split(":")[1],
        )
        ...
        caller_schema_file = get_schema_file(
            args.project_name, caller_schema_name, args
        )
        callee_schema_file = get_schema_file(
            args.project_name, callee_schema_name, args
        )
        ...
        if callee_schema_file is False:
            callee_fragment = ["library", callee_schema_name, callee_signature]
        else:
            callee_schema = {}
            ...
            if callee_fragment == []:
                ...
                for class_ in callee_schema["classes"]:
                    if callee_class_name == class_.split(":")[1]:
                        ...
                        super_class_name += callee_schema["classes"][class_][
                            "implements"
                        ]
                        if callee_schema["classes"][class_]["nested_inside"] != "":
                            super_class_name += [
                                callee_schema["classes"][class_]["nested_inside"].split(
                                    ":"
                                )[1]
                            ]
                        break
                        ...
                        for super_class in super_class_name:
                            for class_ in potential_super_class_schema["classes"]:
                                if super_class == class_.split(":")[1]:
                                    for method in potential_super_class_schema[
                                        "classes"
                                    ][class_]["methods"]:
                                        if callee_method_name == method.split(":")[1]:
                                            callee_fragment = [
        ...
        for class_ in caller_schema["classes"]:
            if caller_class_name == class_.split(":")[1]:
                for method in caller_schema["classes"][class_]["methods"]:
                    if (
                        caller_method_name == method.split(":")[1]
                        and callee_fragment
                        ...
                        caller_schema["classes"][class_]["methods"][method][
                            "calls"
                        ].append(callee_fragment)
                        break

        with open(caller_schema_file, "w") as f:
            json.dump(caller_schema, f, indent=4)

    _annotate_schemas(args)

    # -------------------------------------------------------------------
    # post-process: populate java_types from JavaProject indexer
    # -------------------------------------------------------------------
    _populate_java_types(args)
