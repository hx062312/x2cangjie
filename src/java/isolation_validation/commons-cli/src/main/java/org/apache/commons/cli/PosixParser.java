...
public class PosixParser extends Parser {
    ...
    protected void burstToken(final String token, final boolean stopAtNonOption) {
        for (int i = 1; i < token.length(); i++) {
            ...
            tokens.add("-" + ch);
            currentOption = options_field.getOption(ch);
    ...
    protected String[] flatten(
        ...
        init_();
        this.options_field = options_param;
        ...
        while (it.hasNext()) {
            ...
            if ("-".equals(token) || "--".equals(token)) {
                ...
                final List<String> matchingOpts = options_param.getMatchingOptions(opt);
                ...
                if (matchingOpts.isEmpty()) {
                    ...
                    currentOption = options_param.getOption(matchingOpts.get(0));
                ...
                if (token.length() == 2 || options_param.hasOption(token)) {
                    processOptionToken(token, stopAtNonOption);
                } else if (!options_param.getMatchingOptions(token).isEmpty()) {
                    final List<String> matchingOpts = options_param.getMatchingOptions(token);
                    if (matchingOpts.size() > 1) {
                        throw new AmbiguousOptionException(token, matchingOpts);
                    }
                    final Option opt = options_param.getOption(matchingOpts.get(0));
    ...
    private void processOptionToken(final String token, final boolean stopAtNonOption) {
        if (stopAtNonOption && !options_field.hasOption(token)) {
            eatTheRest = true;
        }

        if (options_field.hasOption(token)) {
            currentOption = options_field.getOption(token);
        }
