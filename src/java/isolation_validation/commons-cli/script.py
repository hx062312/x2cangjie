...
def _snapshot_java_type(snapshot: Any) -> str:
    if not isinstance(snapshot, dict):
        if isinstance(snapshot, bool):
            return "boolean"
        if isinstance(snapshot, int):
            return "int"
        if isinstance(snapshot, float):
            return "double"
        if isinstance(snapshot, str):
            return "String"
        return "Any"

    java_type = snapshot.get("type")
    if isinstance(java_type, str):
        java_type = java_type.replace("$", ".")
        if java_type in _BOXED_TO_PRIMITIVE:
            return _BOXED_TO_PRIMITIVE[java_type]
        if java_type.startswith("src.main.") or java_type.startswith("src.test.") or java_type.startswith("src."):
            return java_type.rsplit(".", 1)[-1]
        if java_type.endswith("[]"):
            return f"{_snapshot_java_type({'type': java_type[:-2]})}[]"
        return java_type.rsplit(".", 1)[-1]
    return "Any"


def _focal_arg_types(method_dict: dict[str, Any]) -> list[str]:
    return [
        _snapshot_java_type(arg_data)
        for _, arg_data in sorted(method_dict.get("Args Initial", []) or [], key=lambda item: int(item[0]))
    ]


def _snapshot_type_key(snapshot: Any) -> str | None:
    if not isinstance(snapshot, dict):
        return None
    java_type = snapshot.get("type")
    if not isinstance(java_type, str) or not java_type:
        return None
    return java_type.replace("$", ".")


def _snapshot_simple_type(snapshot: Any) -> str | None:
    java_type = _snapshot_type_key(snapshot)
    if not java_type:
        return None
    if java_type.endswith("[]"):
...
def _snapshots_type_compatible(left: Any, right: Any) -> bool:
    left_type = _snapshot_type_key(left)
    right_type = _snapshot_type_key(right)
    if left_type and right_type and left_type == right_type:
        return True
    left_simple = _snapshot_simple_type(left)
    right_simple = _snapshot_simple_type(right)
    return bool(left_simple and right_simple and left_simple == right_simple)


def _snapshot_matches_receiver(initial: Any, receiver_initial: Any) -> bool:
    if not _snapshots_type_compatible(initial, receiver_initial):
        return False
    return _deep_equal(initial, receiver_initial)


def _receiver_match_for_dependency(
    method_dict: dict[str, Any],
    focal_methods: list[dict[str, Any]],
) -> tuple[str, str] | None:
    receiver_initial = method_dict.get("Instance Initial")
    if receiver_initial is None:
        return None

    for focal in focal_methods:
        for arg_idx, arg_initial in focal.get("Args Initial", []) or []:
            if _snapshot_matches_receiver(arg_initial, receiver_initial):
                return "arg", str(arg_idx)

        focal_initial = focal.get("Instance Initial")
        if not isinstance(focal_initial, dict):
            continue
        for field_name, field_initial in (focal_initial.get("instance_fields") or {}).items():
            if _snapshot_matches_receiver(field_initial, receiver_initial):
                return "field", str(field_name)

    return None


def _receiver_type_for_dependency(method_dict: dict[str, Any]) -> str:
    receiver_initial = method_dict.get("Instance Initial")
    simple = _snapshot_simple_type(receiver_initial)
    if simple:
        return simple
    return "Any"


def _build_receiver_mock_bindings(
    dependency_methods: list[dict[str, Any]],
    focal_methods: list[dict[str, Any]],
    *,
    instance_var: str = "instance_initial",
) -> tuple[dict[int, str], list[str], list[str], set[int]]:
    """Create mock receivers for bindable instance dependencies.

    Cangjie @On requires the receiver object to be a mock. For dependencies such
    as arg.method(...) or this.injectedField.method(...), generate a mock object
    and bind the focal argument/field to it before emitting @On.
    """
    receiver_vars: dict[int, str] = {}
    setup_lines: list[str] = []
    injection_lines: list[str] = []
    mocked_arg_indices: set[int] = set()
    binding_to_var: dict[tuple[str, str, str], str] = {}

    for dep_index, method_dict in enumerate(dependency_methods):
        if method_dict.get("Instance Initial") is None:
            continue
        match = _receiver_match_for_dependency(method_dict, focal_methods)
        if match is None:
            continue
        receiver_type = _receiver_type_for_dependency(method_dict)
        key = (match[0], match[1], receiver_type)
        receiver_var = binding_to_var.get(key)
        if receiver_var is None:
            receiver_var = f"__dep_receiver_{len(binding_to_var) + 1}"
            binding_to_var[key] = receiver_var
            setup_lines.append(f"let {receiver_var} = mock<{receiver_type}>()")
            if match[0] == "arg":
                injection_lines.append(f"arg_{match[1]} = {receiver_var}")
                try:
                    mocked_arg_indices.add(int(match[1]))
                except ValueError:
                    pass
            else:
                injection_lines.append(f"{instance_var}.{match[1]} = {receiver_var}")
        receiver_vars[dep_index] = receiver_var

    return receiver_vars, setup_lines, injection_lines, mocked_arg_indices


def focal_metadata_lines(method_dict: dict[str, Any]) -> list[str]:
    arg_types = ",".join(_focal_arg_types(method_dict))
    return [
        f"// focal call: {method_dict.get('method_name', 'unknown')}",
        f"// focal argc: {len(method_dict.get('Args Initial', []) or [])}",
        f"// focal arg types: {arg_types}",
    ]
...
def emit_expected_post_state_assertions(
    lines: list[str],
    method_dict: dict[str, Any],
    arg_names: list[str],
    instance_var: str | None,
    *,
    mocked_arg_indices: set[int] | None = None,
) -> None:
    mocked_arg_indices = mocked_arg_indices or set()
    ...
    for arg_idx, arg_data in method_dict.get("Args Final", []):
        arg_index = int(arg_idx)
        if arg_index in mocked_arg_indices:
            continue
        if 0 <= arg_index < len(arg_names):
            lines.append(render_expect_equal(arg_names[arg_index], arg_data, message=f"arg {arg_idx} final snapshot"))
...
def dependency_stub_signature(
    ...
    if not owner_path:
        rendered_method = sanitize_identifier(method_name_base(raw_method or method_name))
        rendered_args = ", ".join(
            render_on_argument_matcher(arg_data)
            for _, arg_data in sorted(method_dict.get("Args Initial", []), key=lambda item: int(item[0]))
        )
        return f"{rendered_method}({rendered_args})", None

    if method_dict.get("Instance Initial") is not None and receiver_var is None:
        return None, "mock-blocker: unbound-instance-method-dependency; receiver is not a generated mock/bindable focal value"
    ...
    owner_expr = receiver_var if receiver_var is not None else cangjie_owner
    return f"{owner_expr}.{rendered_method}({rendered_args})", None
...
def dependency_group_key(
    ...
    signature, reason = dependency_stub_signature(method_dict, project_name, receiver_var)
...
def build_dependency_stub_emission(
    ...
    if receiver_var is not None:
        owner_expr = receiver_var
    elif owner_path:
        owner_expr = strip_project_prefix(to_cangjie_class_path(owner_path), project_name)
    else:
        owner_expr = ""
    if owner_expr:
        signature_expr = f"{owner_expr}.{rendered_method}({', '.join(rendered_args)})"
    else:
        signature_expr = f"{rendered_method}({', '.join(rendered_args)})"
...
def build_dependency_stub_block(
    workflow: list[dict[str, Any]],
    project_name: str,
    receiver_vars: dict[int, str] | None = None,
) -> list[str]:
    lines: list[str] = []
    dependency_methods = [method for method in workflow if method.get("note") != "skip"]
    if not dependency_methods:
        return lines
    receiver_vars = receiver_vars or {}

    grouped: OrderedDict[tuple[str, str], list[dict[str, Any]]] = OrderedDict()
    group_receivers: dict[tuple[str, str], str | None] = {}
    unsupported_lines: list[str] = []

    for dep_index, method_dict in enumerate(dependency_methods):
        receiver_var = receiver_vars.get(dep_index)
        key = dependency_group_key(method_dict, project_name, receiver_var)
        if key is None:
            unsupported_lines.extend(render_dependency_comments(method_dict))
            _, reason = dependency_stub_signature(method_dict, project_name, receiver_var)
            if reason:
                unsupported_lines.append(f"//   mock emission skipped: {reason}")
            continue
        grouped.setdefault(key, []).append(method_dict)
        group_receivers.setdefault(key, receiver_var)

    if grouped:
        lines.append("// dependency stubs emitted via Cangjie @On")
        for group_index, (group_key, method_group) in enumerate(grouped.items(), start=1):
            receiver_var = group_receivers.get(group_key)
            lines.extend(build_dependency_stub_emission(method_group, project_name, group_index, receiver_var))
...
def build_focal_setup(method_dict: dict[str, Any], suffix: str = "") -> tuple[list[str], str | None, list[str]]:
    lines: list[str] = []
    ...
    return lines, instance_var, arg_names


def build_focal_invocation_and_assertions(
    method_dict: dict[str, Any],
    project_name: str,
    arg_names: list[str],
    instance_var: str | None,
    ...
    emit_expected_post_state_assertions(lines, method_dict, arg_names, instance_var, mocked_arg_indices=mocked_arg_indices)
    return lines


def build_focal_body(method_dict: dict[str, Any], project_name: str, suffix: str = "") -> list[str]:
    lines, instance_var, arg_names = build_focal_setup(method_dict, suffix)
    lines.extend(build_focal_invocation_and_assertions(method_dict, project_name, arg_names, instance_var, suffix))
    return lines
...
def build_workflow_body(workflow: list[dict[str, Any]], project_name: str) -> list[str]:
    ...
    focal_header_methods = focal_methods if focal_methods else workflow[-1:]
    for method_dict in focal_header_methods:
        lines.extend(focal_metadata_lines(method_dict))
    if focal_header_methods:
        lines.append("")
    ...
    if focal_methods:
        ...
        setup_lines, instance_var, arg_names = build_focal_setup(focal_method)
        dependency_methods = [method for method in workflow if method.get("note") != "skip"]
        receiver_vars, receiver_setup, receiver_injections, mocked_arg_indices = _build_receiver_mock_bindings(
            dependency_methods,
            focal_methods,
            instance_var=instance_var or "instance_initial",
        )

        lines.extend(setup_lines)
        if receiver_setup:
            lines.extend(receiver_setup)
        if receiver_injections:
            lines.extend(receiver_injections)
        if receiver_setup or receiver_injections:
            lines.append("")

        dependency_stub_lines = build_dependency_stub_block(workflow, project_name, receiver_vars)
        if dependency_stub_lines:
            lines.extend(dependency_stub_lines)
            lines.append("")

        lines.extend(
            build_focal_invocation_and_assertions(
                focal_method,
                project_name,
                arg_names,
                instance_var,
                mocked_arg_indices=mocked_arg_indices,
            )
        )
    else:
