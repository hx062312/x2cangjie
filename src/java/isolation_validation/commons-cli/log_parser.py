...
def parse_logs(input_log_file: str) -> list:
    """日志解析入口：对齐 TRAM，为每个 START OF 调用生成一个 workflow。"""
    ...
    for start_line_index in range(len(lines)):
        ...
        if start_line.startswith("==========START OF"):
            methods_to_mock = []
            mock_indices = {}
            retrieve_mocking_info_for_one_method(
                methods_to_mock,
                mock_indices,
                0,
                len(lines) - 1,
...
def parse_logs_single_workflow(input_log_file: str) -> list:
    """旧策略：一个日志文件只生成一个 workflow，保留作调试 fallback。"""
    with open(input_log_file, 'r') as file:
        lines = file.readlines()
