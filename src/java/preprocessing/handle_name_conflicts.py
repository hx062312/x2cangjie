...
def _extract_package_name(code_text):
    match = re.search(r"^\s*package\s+([A-Za-z_][\w.]*)\s*;", code_text, re.MULTILINE)
...
def _collect_type_refs(node, code, refs):
    if node.type in ("type_identifier", "scoped_type_identifier", "scoped_identifier"):
    ...
    if node.type in TOP_LEVEL_TYPES:
        ...
        if body:
            for child in body.children:
                _collect_top_level_classes(child, code, records, file_info, depth + 1)
        return
...
def _direct_class_members(class_node):
    ...
    if body is None:
        return []
    return [child for child in body.children if child.type not in TOP_LEVEL_TYPES]
...
def _is_this_field_identifier(node, code):
    ...
    return (
        ...
        and extract_text_by_bytes(code, object_node.start_byte, object_node.end_byte)
        == "this"
    )
...
def _collect_project_source_info(output_dir, parser):
    ...
    for java_file in _iter_java_files(output_dir):
        ...
        _collect_top_level_classes(tree.root_node, code, class_records, file_info)
    ...
    for file_path, info in file_infos.items():
        ...
        for imported_fqcn in info["imports"].values():
            if imported_fqcn in fqcn_to_record:
                add_package_edge(
                    graph, src_subpath, fqcn_to_record[imported_fqcn]["raw_subpath"]
                )
        ...
        for ref in info["type_refs"]:
            ...
            if target_fqcn in fqcn_to_record:
                add_package_edge(
                    graph, src_subpath, fqcn_to_record[target_fqcn]["raw_subpath"]
                )
...
def _resolve_flattened_class_renames(class_records, effective_subpaths):
    ...
    for (_effective, _name), records in sorted(
        ...
        for record in sorted(records_to_rename, key=lambda item: item["file_path"]):
...
def _collect_reference_edits(
    node, code, file_info, renames_by_fqcn, simple_to_renamed, edits
):
    node_text = None
            ...
            if parent and parent.type in TOP_LEVEL_TYPES + ("constructor_declaration",):
                for rename_fqcn in simple_to_renamed[old_name]:
                    ...
                    if rename["file_path"] == file_info.get("file_path"):
                        edits.append(
                            (node.start_byte, node.end_byte, rename["new_name"])
                        )
                        break
                return
    ...
    for child in node.children:
        _collect_reference_edits(
            child, code, file_info, renames_by_fqcn, simple_to_renamed, edits
        )
...
def _handle_flattened_subpackage_conflicts(output_dir, parser):
    ...
    for java_file in list(_iter_java_files(output_dir)):
        ...
        for old_fqcn, new_fqcn in sorted(
            fqcn_replacements.items(), key=lambda item: len(item[0]), reverse=True
        ):
        ...
        if simple_name_groups:
            ...
            for old_name, entries in simple_name_groups.items():
                ...
                for old_fqcn, new_fqcn, new_name in entries:
                    ...
                    same_package_fqcn = (
                        f"{file_package}.{old_name}" if file_package else old_name
                    )
                    ...
                    if should_rename:
                        ...
                        # of a qualified name).
                        text = re.sub(
                            rf"(?<!\w\.)\b{re.escape(old_name)}\b", new_name, text
                        )
...
def main(args):
        ...
        for fname in files:
            ...
            def _collect_names(node, depth=0):
                if node.type in (
                    ...
                    name_node = node.child_by_field_name("name")
                    if name_node:
                        cls_name = extract_text_by_bytes(
                            code, name_node.start_byte, name_node.end_byte
                        )
                        ...
                        if depth == 1:
                            ...
                            if superclass:
                                ...
                                if super_name:
                                    file_extends.setdefault(file_path, set()).add(
                                        super_name
                                    )
    ...
    print(
        f"  Found {len(all_names)} class names, "
    ...
    # Step 3: Resolve names and rename using JDT-inspired renamer
    # (replaces regex-based text replacement with AST-level reference tracking)
    print("Step 3/5: Renaming inner classes (AST-based reference tracking)...")
    if all_inner_classes:
        ...
        from src.java.preprocessing.java_renamer.renamer import RenamePlan

        # Index the project once
        print("  Indexing project...")
        project = JavaProject(output_dir)
        project.index()
        ...
        for file_path, ic_list in all_inner_classes.items():
            ...
            with open(file_path, "rb") as f:
                pkg = _extract_package_name(f.read().decode("utf-8", errors="replace"))

            for ic in ic_list:
                old_name = ic["name"]
                outer = ic["outer_class_name"]
                new_name = name_map[(file_path, outer, old_name)]
                if old_name == new_name:
                    continue

                # Build inner class FQCN
                fqcn = f"{pkg}.{outer}.{old_name}" if pkg else f"{outer}.{old_name}"
                sym = project.get_class_symbol(fqcn)
                if sym is None:
                    print(f"  WARNING: {fqcn} not in symbol table, skip")
                    continue

                engine._plan_rename_for_symbol(sym, new_name, plan)
                renamed += 1
                print(f"  {rel}: {outer}.{old_name} -> {new_name}")

        if plan.edit_count > 0:
            engine.apply(plan)
            print(f"  Applied {plan.edit_count} edits across {plan.file_count} files")
        if renamed == 0:
            print("  No inner classes renamed.")
    else:
        print("  Skipped.")

    flattened_renames = _handle_flattened_subpackage_conflicts(output_dir, parser)
