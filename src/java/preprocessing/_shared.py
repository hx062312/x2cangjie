...
def pre_scan_project(output_dir, active_keywords=None, collect_method_decls=False):
    ...
    for root, dirs, files in os.walk(output_dir):
        ...
        for fname in files:
            ...
            _scan_node(
                tree.root_node, code, classes, decls, active_keywords, file_methods
            )
    ...
    return parser, user_classes, file_decls, None


def _scan_node(
    node, code, user_classes, declarations, active_keywords=None, file_methods=None
):
    ...
    if nt in (
        ...
        if name_node:
            user_classes.add(
                extract_text_by_bytes(code, name_node.start_byte, name_node.end_byte)
            )
    ...
    for child in node.children:
        _scan_node(
            child, code, user_classes, declarations, active_keywords, file_methods
        )
